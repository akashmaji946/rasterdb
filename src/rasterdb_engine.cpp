...
#include "op/rasterdb_physical_concat.hpp"
#include "op/rasterdb_physical_cte.hpp"
...
#include "op/rasterdb_physical_grouped_aggregate_merge.hpp"
#include "op/rasterdb_physical_hash_join.hpp"
#include "op/rasterdb_physical_merge_sort.hpp"
...
#include "op/rasterdb_physical_sort_partition.hpp"
#include "op/rasterdb_physical_sort_sample.hpp"
#include "op/rasterdb_physical_table_scan.hpp"
#include "op/rasterdb_physical_top_n.hpp"
#include "op/rasterdb_physical_top_n_merge.hpp"
#include "op/rasterdb_physical_ungrouped_aggregate.hpp"
#include "op/rasterdb_physical_ungrouped_aggregate_merge.hpp"
...
void rasterdb_engine::reset()
{
  rasterdb_physical_plan = nullptr;
  rasterdb_owned_plan.reset();
  rasterdb_root_pipelines.clear();
  root_pipeline_idx = 0;
  total_pipelines   = 0;
  rasterdb_pipelines.clear();
  new_pipeline_breakers.clear();
  rasterdb_scheduled.clear();
  new_scheduled.clear();
}

void rasterdb_engine::insert_repository(
  ...
  op::MemoryBarrierType barrier_type)
{
  ...
  auto& data_repo_manager = context.registered_state->Get<duckdb::RasterDBContext>("rasterdb_state")
  ...
  next_op->add_port(port_id,
                    std::make_unique<op::rasterdb_physical_operator::port>(
                      barrier_type,
  ...
  if (next_op->type == op::RasterDBPhysicalOperatorType::RIGHT_DELIM_JOIN) {
    auto partition_op = next_op->Cast<op::rasterdb_physical_right_delim_join>().partition_join;
    partition_op->add_port(port_id,
                           std::make_unique<op::rasterdb_physical_operator::port>(
                             ...
                             input_pipeline,
                             dependent_pipeline));
  } else if (next_op->type == op::RasterDBPhysicalOperatorType::LEFT_DELIM_JOIN) {
...
void rasterdb_engine::cancel_tasks()
{
  rasterdb_pipelines.clear();
  rasterdb_root_pipelines.clear();
}

duckdb::shared_ptr<pipeline::rasterdb_pipeline> rasterdb_engine::create_child_pipeline(
  pipeline::rasterdb_pipeline& current, op::rasterdb_physical_operator& op)
{
  ...
  auto child_pipeline    = duckdb::make_shared_ptr<pipeline::rasterdb_pipeline>(*this);
...
bool rasterdb_engine::has_result_collector()
{
  return rasterdb_physical_plan->type == op::RasterDBPhysicalOperatorType::RESULT_COLLECTOR;
}

duckdb::unique_ptr<duckdb::QueryResult> rasterdb_engine::get_result()
{
  D_ASSERT(has_result_collector());
  if (!rasterdb_physical_plan) throw duckdb::InvalidInputException("rasterdb_physical_plan is NULL");
  if (rasterdb_physical_plan.get() == NULL)
    throw duckdb::InvalidInputException("rasterdb_physical_plan is NULL");
  auto& result_collector =
    rasterdb_physical_plan.get()->Cast<op::rasterdb_physical_materialized_collector>();
...
void rasterdb_engine::initialize(duckdb::unique_ptr<op::rasterdb_physical_operator> plan)
{
  RASTERDB_LOG_DEBUG("Initializing rasterdb_engine");
  reset();
  rasterdb_owned_plan = std::move(plan);
  initialize_internal(*rasterdb_owned_plan);
}

void rasterdb_engine::execute()
{
  auto sirius_ctx = context.registered_state->Get<duckdb::RasterDBContext>("rasterdb_state");
  if (sirius_ctx == nullptr) {
    throw duckdb::InvalidInputException("RasterDB context is not initialized.");
  }

  // Create the query with the pipeline hashmap
  rasterdb_pipeline_hashmap pipeline_map(new_scheduled);
  ...
  try {
    ...
    /// error to duckdb
    RASTERDB_LOG_ERROR("Error executing query: {}", e.what());
    throw;
  } catch (...) {
    RASTERDB_LOG_ERROR("Unknown error executing query");
    throw;
  }
}

duckdb::unique_ptr<op::rasterdb_physical_operator> rasterdb_engine::construct_sirius_specific_operator(
  op::rasterdb_physical_operator* op)
{
  if (op->type == op::RasterDBPhysicalOperatorType::TABLE_SCAN) {
    auto& scan_physical_op = op->Cast<op::rasterdb_physical_table_scan>();
    if (scan_physical_op.function.name == "parquet_scan" ||
        scan_physical_op.function.name == "read_parquet") {
      return duckdb::make_uniq<op::rasterdb_physical_parquet_scan>(&scan_physical_op);
    } else if (scan_physical_op.function.name == "seq_scan") {
      return duckdb::make_uniq<op::rasterdb_physical_duckdb_scan>(&scan_physical_op);
    } else {
      throw std::runtime_error("Unsupported scan function: " + scan_physical_op.function.name);
    }
  } else if (op->type == op::RasterDBPhysicalOperatorType::HASH_GROUP_BY) {
    auto& group_by_physical_op = op->Cast<op::rasterdb_physical_grouped_aggregate>();
    return duckdb::make_uniq<op::rasterdb_physical_grouped_aggregate_merge>(&group_by_physical_op);
  } else if (op->type == op::RasterDBPhysicalOperatorType::ORDER_BY) {
    auto& order_by_physical_op = op->Cast<op::rasterdb_physical_order>();
    return duckdb::make_uniq<op::rasterdb_physical_merge_sort>(&order_by_physical_op);
  } else if (op->type == op::RasterDBPhysicalOperatorType::TOP_N) {
    auto& topn_physical_op = op->Cast<op::rasterdb_physical_top_n>();
    return duckdb::make_uniq<op::rasterdb_physical_top_n_merge>(&topn_physical_op);
  } else if (op->type == op::RasterDBPhysicalOperatorType::UNGROUPED_AGGREGATE) {
    auto& ungrouped_agg_physical_op = op->Cast<op::rasterdb_physical_ungrouped_aggregate>();
    return duckdb::make_uniq<op::rasterdb_physical_ungrouped_aggregate_merge>(
      &ungrouped_agg_physical_op);
  } else {
    throw duckdb::InternalException("Unsupported operator type" +
                                    RasterdbPhysicalOperatorToString(op->type) +
                                    " for constructing sirius specific operator.");
  }
}

void rasterdb_engine::initialize_internal(op::rasterdb_physical_operator& plan)
{
  // auto &scheduler = TaskScheduler::GetScheduler(context);
  const rasterdb::operator_params& op_params =
    context.registered_state->Get<duckdb::RasterDBContext>("rasterdb_state")
      ->get_config()
      .get_operator_params();
  {
    // lock_guard<mutex> elock(executor_lock);
    rasterdb_physical_plan = &plan;
    ...
    pipeline::rasterdb_pipeline_build_state state;
    auto root_pipeline =
      duckdb::make_shared_ptr<pipeline::rasterdb_meta_pipeline>(*this, state, nullptr);
    root_pipeline->build(*rasterdb_physical_plan);
    root_pipeline->ready();
    ...
    root_pipeline->get_pipelines(rasterdb_root_pipelines, false);
    root_pipeline_idx = 0;

    // collect all meta-pipelines from the root pipeline
    duckdb::vector<duckdb::shared_ptr<pipeline::rasterdb_meta_pipeline>> to_schedule;
    rasterdb_scheduled.clear();
    new_scheduled.clear();
    ...
    RASTERDB_LOG_DEBUG("Total meta pipelines {}", to_schedule.size());
    int schedule_count = 0;
    int meta           = 0;
    while (schedule_count < to_schedule.size()) {
      duckdb::vector<duckdb::shared_ptr<pipeline::rasterdb_meta_pipeline>> children;
      ...
      // already scheduled
      if (find(rasterdb_scheduled.begin(), rasterdb_scheduled.end(), base_pipeline) !=
          rasterdb_scheduled.end()) {
        should_schedule = false;
      } else {
        // check if all children are scheduled
        for (auto& child : children) {
          if (find(rasterdb_scheduled.begin(), rasterdb_scheduled.end(), child->get_base_pipeline()) ==
              rasterdb_scheduled.end()) {
            should_schedule = false;
            break;
          }
        ...
        for (int dep = 0; dep < base_pipeline->dependencies.size(); dep++) {
          if (find(rasterdb_scheduled.begin(),
                   rasterdb_scheduled.end(),
                   base_pipeline->dependencies[dep]) == rasterdb_scheduled.end()) {
            should_schedule = false;
            break;
          }
        }
      ...
      if (should_schedule) {
        duckdb::vector<duckdb::shared_ptr<pipeline::rasterdb_pipeline>> pipeline_inside;
        ...
        for (int pipeline_idx = 0; pipeline_idx < pipeline_inside.size(); pipeline_idx++) {
          ...
          if (pipeline_inside[pipeline_idx]->source->type ==
              op::RasterDBPhysicalOperatorType::HASH_JOIN) {
            auto& temp =
              pipeline_inside[pipeline_idx]->source.get()->Cast<op::rasterdb_physical_hash_join>();
            if (temp.join_type == duckdb::JoinType::RIGHT ||
              ...
              // if (!duckdb::Config::MODIFIED_PIPELINE) rasterdb_scheduled.push_back(pipeline);
            }
            continue;
          } else {
            rasterdb_scheduled.push_back(pipeline);
          }
        }
        schedule_count++;
      }
    ...
    duckdb::vector<duckdb::shared_ptr<pipeline::rasterdb_pipeline>> copied_scheduled;
    for (size_t i = 0; i < rasterdb_scheduled.size(); i++) {
      auto copied_pipeline = duckdb::make_shared_ptr<pipeline::rasterdb_pipeline>(*this);
      // copy source
      copied_pipeline->source = rasterdb_scheduled[i]->source;
      // copy operators
      for (size_t j = 0; j < rasterdb_scheduled[i]->operators.size(); j++) {
        copied_pipeline->operators.push_back(rasterdb_scheduled[i]->operators[j]);
      }
      // copy sink
      copied_pipeline->sink = rasterdb_scheduled[i]->sink;
    ...
    auto& data_repo_manager = context.registered_state->Get<duckdb::RasterDBContext>("rasterdb_state")
                                ->get_data_repository_manager();
    std::unordered_map<const op::rasterdb_physical_operator*,
                       duckdb::vector<duckdb::shared_ptr<pipeline::rasterdb_pipeline>>>
      source_to_pipelines;
    ...
    for (size_t i = 0; i < copied_scheduled.size(); i++) {
      ...
      if (current_pipeline->source->type == op::RasterDBPhysicalOperatorType::TABLE_SCAN) {
        auto& scan_op = current_pipeline->get_source()->Cast<op::rasterdb_physical_table_scan>();
        if (scan_op.function.name == "seq_scan" || scan_op.function.name == "parquet_scan" ||
            scan_op.function.name == "read_parquet") {
          auto new_pipeline = duckdb::make_shared_ptr<pipeline::rasterdb_pipeline>(*this);
      ...
      duckdb::shared_ptr<pipeline::rasterdb_pipeline> previous_pipeline = nullptr;
      op::rasterdb_physical_concat* prev_concat_ptr                     = nullptr;
      ...
      for (duckdb::idx_t op_idx = 0; op_idx < current_pipeline->operators.size(); op_idx++) {
        if (current_pipeline->operators[op_idx].get().type ==
            ...
            current_pipeline->operators[op_idx].get().type ==
              op::RasterDBPhysicalOperatorType::NESTED_LOOP_JOIN) {
          join_positions.push_back(op_idx);
        }
      }

      bool group_agg_sink = false;
      if (current_pipeline->sink->type == op::RasterDBPhysicalOperatorType::HASH_GROUP_BY ||
          current_pipeline->sink->type == op::RasterDBPhysicalOperatorType::UNGROUPED_AGGREGATE) {
        group_agg_sink = true;
      }

      bool order_by_sink =
        (current_pipeline->sink->type == op::RasterDBPhysicalOperatorType::ORDER_BY);

      bool top_n_sink = (current_pipeline->sink->type == op::RasterDBPhysicalOperatorType::TOP_N);

      bool join_sink = false;
      if (current_pipeline->sink->type == op::RasterDBPhysicalOperatorType::HASH_JOIN ||
          current_pipeline->sink->type == op::RasterDBPhysicalOperatorType::NESTED_LOOP_JOIN) {
        join_sink = true;
      }

      bool right_left_delim_join_sink = false;
      if (current_pipeline->sink->type == op::RasterDBPhysicalOperatorType::LEFT_DELIM_JOIN ||
          current_pipeline->sink->type == op::RasterDBPhysicalOperatorType::RIGHT_DELIM_JOIN) {
      ...
      if (!join_positions.empty()) {
        for (size_t hj_idx = 0; hj_idx < join_positions.size(); hj_idx++) {
          ...
          duckdb::unique_ptr<op::rasterdb_physical_concat> concat_op;

          // Create a PARTITION and CONCAT operator
          if (join_pos == 0) {
            concat_op = make_uniq<op::rasterdb_physical_concat>(
            ...
            auto partition_op = make_uniq<op::rasterdb_physical_partition>(
              current_pipeline->get_source()->types,
              current_pipeline->get_source()->estimated_cardinality,
              concat_op.get(),
              false,
          ...
          op::rasterdb_physical_partition* partition_ptr =
            static_cast<op::rasterdb_physical_partition*>(new_pipeline_breakers.back().get());
          ...
          if (join_pos > 0) {
            ...
            auto partition_pipeline    = duckdb::make_shared_ptr<pipeline::rasterdb_pipeline>(*this);
          ...
          auto concat_pipeline    = duckdb::make_shared_ptr<pipeline::rasterdb_pipeline>(*this);
          concat_pipeline->source = partition_ptr;
          concat_pipeline->sink   = concat_op.get();

          new_pipeline_breakers.push_back(std::move(concat_op));
          op::rasterdb_physical_concat* concat_ptr =
            static_cast<op::rasterdb_physical_concat*>(new_pipeline_breakers.back().get());
      ...
      if (join_sink) {
        ...
        duckdb::unique_ptr<op::rasterdb_physical_partition> partition_op;
        duckdb::unique_ptr<op::rasterdb_physical_concat> concat_op;
        ...
        if (current_pipeline->operators.size() == 0) {
          ...
          concat_op = make_uniq<op::rasterdb_physical_concat>(
            ...
            true,
            op_params.concat_batch_bytes);
          partition_op = make_uniq<op::rasterdb_physical_partition>(
            current_pipeline->get_source()->types,
            current_pipeline->get_source()->estimated_cardinality,
            concat_op.get(),
            true,
        ...
        op::rasterdb_physical_partition* partition_ptr =
          static_cast<op::rasterdb_physical_partition*>(partition_op.get());

        if (current_pipeline->operators.size() > 0) {
          // Last op before HASH_JOIN becomes the sink
          op::rasterdb_physical_operator* last_op_ptr = &current_pipeline->operators.back().get();
      ...
      if (group_agg_sink) {
        ...
        if (group_agg_op->type == op::RasterDBPhysicalOperatorType::HASH_GROUP_BY) {
          ...
          auto merge_pipeline    = duckdb::make_shared_ptr<pipeline::rasterdb_pipeline>(*this);
      ...
      if (order_by_sink) {
        ...
        auto* order_ptr = static_cast<op::rasterdb_physical_order*>(order_op.get());
        ...
        auto sample_op   = duckdb::make_uniq<op::rasterdb_physical_sort_sample>(order_ptr);
        ...
        auto sample_pipeline    = duckdb::make_shared_ptr<pipeline::rasterdb_pipeline>(*this);
        ...
        auto partition_op   = duckdb::make_uniq<op::rasterdb_physical_sort_partition>(order_ptr);
        ...
        auto merge_op   = duckdb::make_uniq<op::rasterdb_physical_merge_sort>(order_ptr);
      ...
      if (top_n_sink) {
        ...
        auto* topn_ptr = static_cast<op::rasterdb_physical_top_n*>(top_n_op.get());

        // Pipeline A: current pipeline keeps TOP_N as sink
        new_scheduled.push_back(current_pipeline);

        // Create MERGE_TOP_N operator
        auto merge_op = duckdb::unique_ptr<op::rasterdb_physical_top_n_merge>(
          new op::rasterdb_physical_top_n_merge(topn_ptr));
        auto* merge_ptr = merge_op.get();
      ...
      if (right_left_delim_join_sink) {
        ...
        auto& join_op     = delim_join->Cast<op::rasterdb_physical_delim_join>().join;
        auto& distinct_op = delim_join->Cast<op::rasterdb_physical_delim_join>().distinct;

        duckdb::unique_ptr<op::rasterdb_physical_partition> partition_join;
        if (delim_join->type == op::RasterDBPhysicalOperatorType::RIGHT_DELIM_JOIN) {
          if (current_pipeline->operators.size() == 0) {
            partition_join = make_uniq<op::rasterdb_physical_partition>(
              current_pipeline->get_source()->types,
              current_pipeline->get_source()->estimated_cardinality,
              join_op.get(),
              true,
          ...
          delim_join->Cast<op::rasterdb_physical_right_delim_join>().partition_join =
            static_cast<op::rasterdb_physical_partition*>(partition_join.get());
        } else if (delim_join->type == op::RasterDBPhysicalOperatorType::LEFT_DELIM_JOIN) {
          delim_join->Cast<op::rasterdb_physical_left_delim_join>().column_data_scan =
            static_cast<op::rasterdb_physical_column_data_scan*>(join_op->children[0].get());
        }

        // Create partition_distinct — external to delim join, in its own pipeline
        auto partition_distinct =
          make_uniq<op::rasterdb_physical_partition>(distinct_op->types,
                                                   distinct_op->estimated_cardinality,
                                                   distinct_op.get(),
                                                   false,
                                                   op_params.hash_partition_bytes);
        op::rasterdb_physical_partition* partition_distinct_ptr =
          static_cast<op::rasterdb_physical_partition*>(partition_distinct.get());

        // The pipeline that contains the delim join as sink
        duckdb::shared_ptr<pipeline::rasterdb_pipeline> delim_join_pipeline;
        ...
        if (delim_join->type == op::RasterDBPhysicalOperatorType::RIGHT_DELIM_JOIN &&
          ...
          auto delim_pipeline    = duckdb::make_shared_ptr<pipeline::rasterdb_pipeline>(*this);
        ...
        if (delim_join->type == op::RasterDBPhysicalOperatorType::RIGHT_DELIM_JOIN) {
          ...
          auto concat_pipeline = duckdb::make_shared_ptr<pipeline::rasterdb_pipeline>(*this);
          duckdb::unique_ptr<op::rasterdb_physical_concat> concat_op =
            make_uniq<op::rasterdb_physical_concat>(partition_join.get()->types,
        ...
        auto partition_distinct_pipeline =
          duckdb::make_shared_ptr<pipeline::rasterdb_pipeline>(*this);
    ...
    for (size_t i = 0; i < new_scheduled.size(); i++) {
      if (new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::MERGE_GROUP_BY ||
          new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::MERGE_SORT ||
          new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::MERGE_TOP_N ||
          new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::MERGE_AGGREGATE) {
      ...
      } else if (new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::CTE) {
        auto& cte_op             = new_scheduled[i]->get_sink()->Cast<op::rasterdb_physical_cte>();
        std::string_view port_id = "default";
      ...
      } else if (new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::RIGHT_DELIM_JOIN) {
        auto delim_join     = new_scheduled[i]->get_sink();
        auto& right_delim   = delim_join->Cast<op::rasterdb_physical_right_delim_join>();
      ...
      } else if (new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::LEFT_DELIM_JOIN) {
        auto delim_join       = new_scheduled[i]->get_sink();
        auto& left_delim      = delim_join->Cast<op::rasterdb_physical_left_delim_join>();
      ...
      } else if (new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::CONCAT) {
        auto& concat             = new_scheduled[i]->get_sink()->Cast<op::rasterdb_physical_concat>();
        ...
        if (concat.is_build_concat()) {
          ...
          op::rasterdb_physical_operator* hash_join_op = concat.get_parent_op();
          bool found                                 = false;
      ...
      } else if (new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::PARTITION ||
                 new_scheduled[i]->sink->type ==
                   op::RasterDBPhysicalOperatorType::UNGROUPED_AGGREGATE ||
                 new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::TOP_N ||
                 new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::MERGE_SORT ||
                 new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::SORT_PARTITION) {
        for (auto dependent_pipeline : source_to_pipelines[new_scheduled[i]->get_sink().get()]) {
          // if the source is CONCAT, then use partial barrier type
          if ((dependent_pipeline->get_sink()->type == op::RasterDBPhysicalOperatorType::CONCAT &&
               dependent_pipeline->get_operators().size() == 0) ||
              (dependent_pipeline->get_operators().size() > 0 &&
               dependent_pipeline->get_operators()[0].get().type ==
                 op::RasterDBPhysicalOperatorType::CONCAT)) {
            insert_repository(
      ...
      } else if (new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::ORDER_BY ||
                 new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::SORT_SAMPLE) {
      ...
      } else if (new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::DUCKDB_SCAN ||
                 new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::PARQUET_SCAN) {
        for (auto dependent_pipeline : source_to_pipelines[new_scheduled[i]->get_sink().get()]) {
          auto next_op             = dependent_pipeline->get_operators().size() == 0
                                       ? dependent_pipeline->get_sink().get()
                                       : &dependent_pipeline->get_operators()[0].get();
      ...
      } else if (new_scheduled[i]->sink->type == op::RasterDBPhysicalOperatorType::RESULT_COLLECTOR) {
    ...
    for (size_t i = 0; i < new_scheduled.size(); i++) {
      ...
      if (new_scheduled[i]->sink.get()) {
        if (new_scheduled[i]->sink.get()->type ==
            op::RasterDBPhysicalOperatorType::RIGHT_DELIM_JOIN) {
          auto& delim_join =
            new_scheduled[i]->sink.get()->Cast<op::rasterdb_physical_right_delim_join>();
          ...
          for (auto& [next_op, port_id] : partition_join->get_next_port_after_sink()) {
            if (next_op->get_port(port_id)->dest_pipeline) {
              new_scheduled[i]->parents.push_back(
                duckdb::weak_ptr<rasterdb::pipeline::rasterdb_pipeline>(
              ...
              new_scheduled[i]->parents.push_back(
                duckdb::weak_ptr<rasterdb::pipeline::rasterdb_pipeline>(
                  next_op->get_port(port_id)->dest_pipeline));
                   ...
                   op::RasterDBPhysicalOperatorType::LEFT_DELIM_JOIN) {
          auto& delim_join =
            new_scheduled[i]->sink.get()->Cast<op::rasterdb_physical_left_delim_join>();
          auto* distinct_op     = delim_join.distinct.get();
    ...
    for (size_t i = 0; i < new_scheduled.size(); i++) {
      ...
      // concat (partition)
      if (new_scheduled[i]->source->type == op::RasterDBPhysicalOperatorType::HASH_JOIN) {
        ...
        if (build_partition_pipeline->get_sink()->type ==
          ...
          auto& right_delim_join_op =
            build_partition_pipeline->get_sink()->Cast<op::rasterdb_physical_right_delim_join>();
          auto build_partition_op = right_delim_join_op.partition_join;
          auto& probe_partition_op =
            probe_partition_pipeline->get_sink()->Cast<op::rasterdb_physical_partition>();
          ...
          auto& build_partition_op =
            build_partition_pipeline->get_sink()->Cast<op::rasterdb_physical_partition>();
          auto& probe_partition_op =
            probe_partition_pipeline->get_sink()->Cast<op::rasterdb_physical_partition>();
          build_partition_op.set_sibling_partition_op(&probe_partition_op);
          probe_partition_op.set_sibling_partition_op(&build_partition_op);
    ...
    for (size_t i = 0; i < new_scheduled.size(); i++) {
      ...
      for (size_t j = 0; j < pipeline->operators.size(); j++) {
        ...
        RASTERDB_LOG_INFO("    Operator[{}]: {} (id={})", j, op.get_name(), op.get_operator_id());
      }
      ...
      if (pipeline->operators.size() > 0) {
        ...
        if (first_op.type == op::RasterDBPhysicalOperatorType::HASH_JOIN ||
            first_op.type == op::RasterDBPhysicalOperatorType::NESTED_LOOP_JOIN) {
          // Joins have "default" and "build" ports
          auto* default_port = first_op.get_port("default");
          if (default_port) {
            RASTERDB_LOG_INFO("    Port 'default': barrier_type={}, repo={}",
                            static_cast<int>(default_port->type),
                            static_cast<void*>(default_port->repo));
          }
          auto* build_port = first_op.get_port("build");
          if (build_port) {
            RASTERDB_LOG_INFO("    Port 'build': barrier_type={}, repo={}",
                            static_cast<int>(build_port->type),
                            static_cast<void*>(build_port->repo));
          }
        } else if (first_op.type == op::RasterDBPhysicalOperatorType::TABLE_SCAN) {
          const auto& scan_name = first_op.Cast<op::rasterdb_physical_table_scan>().function.name;
          ...
          if (scan_port) {
            RASTERDB_LOG_INFO("    Port 'scan': barrier_type={}, repo={}",
                            static_cast<int>(scan_port->type),
                            static_cast<void*>(scan_port->repo));
          }
        } else if (first_op.type == op::RasterDBPhysicalOperatorType::DUCKDB_SCAN ||
                   first_op.type == op::RasterDBPhysicalOperatorType::PARQUET_SCAN) {
        ...
        RASTERDB_LOG_INFO("  No operators in pipeline - checking sink ports");
        auto* sink = pipeline->sink.get();

        if (sink->type == op::RasterDBPhysicalOperatorType::HASH_JOIN ||
            sink->type == op::RasterDBPhysicalOperatorType::NESTED_LOOP_JOIN) {
        ...
        } else if (sink->type == op::RasterDBPhysicalOperatorType::TABLE_SCAN) {
        ...
        } else if (sink->type == op::RasterDBPhysicalOperatorType::DUCKDB_SCAN ||
                   sink->type == op::RasterDBPhysicalOperatorType::PARQUET_SCAN) {
          // ignore DUCKDB_SCAN  and PARQUET_SCAN since it doesn't have port
        } else {
          auto* default_port = sink->get_port("default");
          if (default_port) {
      ...
      for (auto& next_port : pipeline->sink->get_next_port_after_sink()) {
        ...
        if (port) {
          RASTERDB_LOG_INFO("      Port barrier_type={}, repo={}",
      ...
      if (pipeline->sink->type == op::RasterDBPhysicalOperatorType::RIGHT_DELIM_JOIN ||
          pipeline->sink->type == op::RasterDBPhysicalOperatorType::LEFT_DELIM_JOIN) {
        auto delim_join = pipeline->get_sink();

        if (pipeline->sink->type == op::RasterDBPhysicalOperatorType::RIGHT_DELIM_JOIN) {
          auto partition_join =
            delim_join->Cast<op::rasterdb_physical_right_delim_join>().partition_join;
          RASTERDB_LOG_INFO("  Partition Join next operators:");
          for (auto& next_port : partition_join->get_next_port_after_sink()) {
            RASTERDB_LOG_INFO("    Next Op: {} (id={}), Port: '{}' Repo:'{}'",
          ...
          auto distinct_op =
            delim_join->Cast<op::rasterdb_physical_right_delim_join>().distinct.get();
          RASTERDB_LOG_INFO("  Distinct next operators:");
        ...
        if (pipeline->sink->type == op::RasterDBPhysicalOperatorType::LEFT_DELIM_JOIN) {
          auto column_data_scan =
            delim_join->Cast<op::rasterdb_physical_left_delim_join>().column_data_scan;
          RASTERDB_LOG_INFO("  Column Data Scan next operators:");
          ...
          auto distinct_op = delim_join->Cast<op::rasterdb_physical_left_delim_join>().distinct.get();
          RASTERDB_LOG_INFO("  Partition Distinct next operators:");
          for (auto& next_port : distinct_op->get_next_port_after_sink()) {
            RASTERDB_LOG_INFO("    Next Op: {} (id={}), Port: '{}' Repo:'{}'",
                            next_port.first->get_name(),
                            next_port.first->get_operator_id(),
      ...
      RASTERDB_LOG_INFO("");  // Blank line between pipelines
    }
    RASTERDB_LOG_INFO("=== END DETAILED PIPELINE DEBUG INFO ===\n");

    // Flush immediately to ensure all debug output is written synchronously
    // spdlog::default_logger()->flush();

    // create invalid operators
    auto invalid_op = make_uniq<op::rasterdb_physical_operator>(
      op::RasterDBPhysicalOperatorType::INVALID, duckdb::vector<duckdb::LogicalType>{}, 0);

    // collect all pipelines from the root pipelines (recursively) for the progress bar and verify
    // them
    root_pipeline->get_pipelines(rasterdb_pipelines, true);
    RASTERDB_LOG_DEBUG("total_pipelines = {}", rasterdb_pipelines.size());
  }
}
