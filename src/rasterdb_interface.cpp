...
#include "rasterdb_interface.hpp"
...
rasterdb_interface::rasterdb_interface(duckdb::ClientContext& client_context)
  : client_context(client_context) {};

void rasterdb_interface::sirius_process_error(duckdb::ErrorData& error,
...
template <class T>
duckdb::unique_ptr<T> rasterdb_interface::sirius_error_result(duckdb::ErrorData error,
...
void rasterdb_interface::check_executable_internal(duckdb::PendingQueryResult& pending)
{
...
void rasterdb_interface::begin_query_internal(const duckdb::string& query)
{
...
duckdb::unique_ptr<duckdb::QueryResult> rasterdb_interface::fetch_result_internal(
  duckdb::PendingQueryResult& pending)
{
  D_ASSERT(sirius_active_query);
  D_ASSERT(sirius_active_query->is_open_result(pending));
  D_ASSERT(sirius_active_query->sirius_prepared->prepared);
  auto& engine   = get_rasterdb_engine();
  auto& prepared = *sirius_active_query->sirius_prepared->prepared;
  duckdb::unique_ptr<duckdb::QueryResult> result;
  D_ASSERT(engine.has_result_collector());
  RASTERDB_LOG_DEBUG("Fetching result from GPU executor");
  result = engine.get_result();
  cleanup_internal(result.get(), false);
  return result;
}

void rasterdb_interface::cleanup_internal(duckdb::BaseQueryResult* result,
...
duckdb::ErrorData rasterdb_interface::end_query_internal(bool success, bool invalidate_transaction)
{
  sirius_active_query->progress_bar.reset();
...
rasterdb_interface::sirius_pending_statement_or_prepared_statement(
...
duckdb::unique_ptr<duckdb::PendingQueryResult> rasterdb_interface::sirius_pending_statement_internal(
  duckdb::ClientContext& context,
  duckdb::shared_ptr<sirius_prepared_statement_data>& statement_p,
  const duckdb::PendingQueryParameters& parameters)
{
  ...
  duckdb::unique_ptr<rasterdb_engine> temp = duckdb::make_uniq<rasterdb_engine>(context, *this);
  auto prop                              = temp->context.GetClientProperties();
  sirius_active_query->engine            = std::move(temp);
  auto& engine                           = get_rasterdb_engine();
  bool stream_result                     = false;

  duckdb::unique_ptr<op::rasterdb_physical_result_collector> sirius_collector =
    duckdb::make_uniq_base<op::rasterdb_physical_result_collector,
                           op::rasterdb_physical_materialized_collector>(*statement_p,
                                                                       client_context);
  if (sirius_collector->type != op::RasterDBPhysicalOperatorType::RESULT_COLLECTOR) {
    return sirius_error_result<duckdb::PendingQueryResult>(
      duckdb::ErrorData("Error in sirius_pending_statement_internal"));
  }
  D_ASSERT(sirius_collector->type == op::RasterDBPhysicalOperatorType::RESULT_COLLECTOR);
...
duckdb::unique_ptr<duckdb::QueryResult> rasterdb_interface::sirius_execute_pending_query_result(
  duckdb::PendingQueryResult& pending)
{
  D_ASSERT(sirius_active_query->is_open_result(pending));
  ...
  auto& engine = get_rasterdb_engine();
  try {
    RASTERDB_LOG_DEBUG("Executing rasterdb_engine");
    engine.execute();
    RASTERDB_LOG_DEBUG("Done executing rasterdb_engine");
  } catch (std::exception& e) {
    duckdb::ErrorData error(e);
    RASTERDB_LOG_ERROR("Error in sirius_execute_pending_query_result: {}", error.RawMessage());
  ...
  RASTERDB_LOG_DEBUG("Done sirius_execute_pending_query_result");
  auto result = fetch_result_internal(pending);
  return result;
}

// This function is based on ClientContext::Query
duckdb::unique_ptr<duckdb::QueryResult> rasterdb_interface::sirius_execute_query(
  duckdb::ClientContext& context,
  const duckdb::string& query,
  duckdb::shared_ptr<sirius_prepared_statement_data>& statement_p,
  const duckdb::PendingQueryParameters& parameters)
{
  ...
  RASTERDB_LOG_DEBUG("Done sirius_execute_query");
  return current_result;
};

rasterdb::rasterdb_engine& rasterdb_interface::get_rasterdb_engine()
{
  D_ASSERT(sirius_active_query);
