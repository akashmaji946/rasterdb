...
class rasterdb_physical_sort_sample;

class rasterdb_physical_sort_partition : public rasterdb_physical_operator {
 public:
  static constexpr const RasterDBPhysicalOperatorType TYPE =
    RasterDBPhysicalOperatorType::SORT_PARTITION;

 public:
  rasterdb_physical_sort_partition(rasterdb_physical_order* order_by);

  rasterdb_physical_sort_partition(duckdb::vector<duckdb::LogicalType> types,
 ...
 public:
  ...
  void set_sample_op(rasterdb_physical_sort_sample* sample) { _sample_op = sample; }

  //! Get the sample operator
  rasterdb_physical_sort_sample* get_sample_op() const { return _sample_op; }

 private:
  rasterdb_physical_sort_sample* _sample_op = nullptr;
};
