...
class rasterdb_physical_partition : public rasterdb_physical_operator {
 public:
  static constexpr const RasterDBPhysicalOperatorType TYPE = RasterDBPhysicalOperatorType::PARTITION;

  explicit rasterdb_physical_partition(
    duckdb::vector<duckdb::LogicalType> types,
    duckdb::idx_t estimated_cardinality,
    rasterdb_physical_operator* parent_op,
    bool is_build                 = false,
    uint64_t hash_partition_bytes = rasterdb::config::DEFAULT_HASH_PARTITION_BYTES);
  ...
  [[nodiscard]] rasterdb_physical_operator* get_parent_op() const { return _parent_op; }

  [[nodiscard]] rasterdb_physical_operator* get_sibling_partition_op() const
  {
  ...
  void set_sibling_partition_op(rasterdb_physical_operator* sibling_partition_op)
  {
 ...
 private:
  void get_partition_keys_and_type(rasterdb_physical_operator* op, bool is_build = false);
  int determine_num_partitions();
  rasterdb_physical_operator* _parent_op            = nullptr;
  rasterdb_physical_operator* _sibling_partition_op = nullptr;
