...
class rasterdb_physical_duckdb_scan : public rasterdb_physical_operator {
 public:
  static constexpr const RasterDBPhysicalOperatorType TYPE = RasterDBPhysicalOperatorType::DUCKDB_SCAN;

 public:
  rasterdb_physical_duckdb_scan(rasterdb_physical_table_scan* table_scan);

  //! Table scan that immediately projects out filter columns that are unused in the remainder of
  //! the query plan
  rasterdb_physical_duckdb_scan(duckdb::vector<duckdb::LogicalType> types,
