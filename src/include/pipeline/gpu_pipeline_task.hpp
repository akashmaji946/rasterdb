...
public:
     ...
     */
    explicit gpu_pipeline_task_global_state(data_repository_manager& data_repo_mgr, task_completion_message_queue& message_queue) : 
...
class gpu_pipeline_task_local_state : public itask_local_state {
    public:
         ...
         */
        explicit gpu_pipeline_task_local_state(uint64_t task_id, uint64_t pipeline_id, duckdb::shared_ptr<duckdb::GPUPipeline> pipeline, 
            sirius::vector<sirius::unique_ptr<data_batch_view>> batch_views) : 
            _task_id(task_id), _pipeline_id(pipeline_id), _pipeline(pipeline), _batch_views(batch_views) {}
 ...
 public:
    ...
    const duckdb::GPUPipeline* get_pipeline() const;
      ...
      */
     void mark_task_completion() override;
