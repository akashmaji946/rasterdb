 ...
 */
class rasterdb_pipeline_task_global_state : public rasterdb::parallel::itask_global_state {
 public:
  /**
   * @brief Construct a new rasterdb_pipeline_task_global_state object
   *
   * @param pipeline Shared pointer to the GPU pipeline to execute
   */
  explicit rasterdb_pipeline_task_global_state(duckdb::shared_ptr<rasterdb_pipeline> pipeline)
    : _pipeline(std::move(pipeline))
  {
  }

  [[nodiscard]] const rasterdb_pipeline* get_pipeline() const { return _pipeline.get(); }

  [[nodiscard]] rasterdb_pipeline* get_pipeline() { return _pipeline.get(); }

  [[nodiscard]] size_t get_pipeline_id() const { return _pipeline->get_pipeline_id(); }

 private:
  duckdb::shared_ptr<rasterdb_pipeline> _pipeline;  ///< Shared pointer to the GPU pipeline to execute
};
...
class rasterdb_pipeline_task_local_state : public parallel::itask_local_state {
 public:
  /**
   * @brief Destructor for proper cleanup of derived classes.
   */
  ~rasterdb_pipeline_task_local_state() override = default;
 ...
 protected:
   ...
   */
  rasterdb_pipeline_task_local_state() = default;
