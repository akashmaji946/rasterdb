...
class rasterdb_pipeline_itask : public parallel::itask {
 public:
  /**
   * @brief Destructor for proper cleanup of derived classes.
   */
  ~rasterdb_pipeline_itask() override = default;
  ...
  virtual std::vector<op::rasterdb_physical_operator*> get_output_consumers() = 0;
  ...
  {
    return _global_state->cast<rasterdb_pipeline_task_global_state>().get_pipeline_id();
  }
 ...
 protected:
   ...
   */
  rasterdb_pipeline_itask(std::unique_ptr<rasterdb_pipeline_task_local_state> local_state,
