...
class rasterdb_pipeline_build_state {
 ...
 public:
  ...
  duckdb::reference_map_t<const op::rasterdb_physical_operator, duckdb::reference<rasterdb_pipeline>>
 ...
 public:
  void set_pipeline_source(rasterdb_pipeline& pipeline, op::rasterdb_physical_operator& op);
  void set_pipeline_sink(rasterdb_pipeline& pipeline,
  ...
  void add_pipeline_operator(rasterdb_pipeline& pipeline, op::rasterdb_physical_operator& op);
  duckdb::shared_ptr<rasterdb_pipeline> create_child_pipeline(rasterdb_engine& engine,
                                                            rasterdb_pipeline& pipeline,
                                                            op::rasterdb_physical_operator& op);

  duckdb::optional_ptr<op::rasterdb_physical_operator> get_pipeline_source(rasterdb_pipeline& pipeline);
  duckdb::optional_ptr<op::rasterdb_physical_operator> get_pipeline_sink(rasterdb_pipeline& pipeline);
  duckdb::vector<duckdb::reference<op::rasterdb_physical_operator>> get_pipeline_operators(
    rasterdb_pipeline& pipeline);
};

//! The rasterdb_pipeline class represents an execution pipeline starting at a
class rasterdb_pipeline : public duckdb::enable_shared_from_this<rasterdb_pipeline> {
  friend class ::rasterdb::rasterdb_engine;
  friend class rasterdb_pipeline_build_state;
  friend class rasterdb_meta_pipeline;

 public:
  explicit rasterdb_pipeline(rasterdb_engine& engine);
  virtual ~rasterdb_pipeline() = default;

 public:
  duckdb::ClientContext& get_client_context();

  void add_dependency(duckdb::shared_ptr<rasterdb_pipeline>& pipeline);
  ...
  // duckdb::vector<duckdb::reference<op::rasterdb_physical_operator>> get_all_operators();

  // duckdb::vector<duckdb::const_reference<op::rasterdb_physical_operator>> get_all_operators()
  // const;

  //! Returns a list of all operators (including source and sink) involved in this pipeline
  duckdb::vector<duckdb::reference<op::rasterdb_physical_operator>> get_operators();
  duckdb::vector<duckdb::const_reference<op::rasterdb_physical_operator>> get_operators() const;

  duckdb::optional_ptr<op::rasterdb_physical_operator> get_sink() { return sink; }

  duckdb::optional_ptr<op::rasterdb_physical_operator> get_sink() const noexcept { return sink; }

  duckdb::optional_ptr<op::rasterdb_physical_operator> get_source() { return source; }

  duckdb::optional_ptr<op::rasterdb_physical_operator> get_source() const noexcept { return source; }
  ...
  std::vector<rasterdb_pipeline*> get_parents() const;

  std::vector<op::rasterdb_physical_operator*> get_output_consumers() const;
  ...
  // duckdb::vector<std::weak_ptr<rasterdb_pipeline>> dependencies;
  duckdb::vector<duckdb::shared_ptr<rasterdb_pipeline>> dependencies;
 ...
 private:
  ...
  duckdb::optional_ptr<op::rasterdb_physical_operator> source;
  ...
  duckdb::vector<duckdb::weak_ptr<rasterdb_pipeline>> parents;
