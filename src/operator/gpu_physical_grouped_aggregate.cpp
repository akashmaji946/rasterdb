...
ResolveTypeGroupByAggregateExpression(vector<shared_ptr<GPUColumn>> &group_by_keys, vector<shared_ptr<GPUColumn>> &aggregate_keys, GPUBufferManager* gpuBufferManager, const vector<unique_ptr<Expression>> &aggregates, int num_group_keys) {
	...
	for (idx_t group = 0; group < num_group_keys; group++) {
		...
		group_by_keys[group] = make_shared_ptr<GPUColumn>(count[0], group_by_keys[group]->data_wrapper.type, reinterpret_cast<uint8_t*>(group_by_data[group]), nullptr);
	...
	for (int agg_idx = 0; agg_idx < aggregates.size(); agg_idx++) {
		...
		if (expr.function.name.compare("count_star") == 0 || expr.function.name.compare("count") == 0) {
			aggregate_keys[agg_idx] = make_shared_ptr<GPUColumn>(count[0], GPUColumnType(GPUColumnTypeId::INT64), reinterpret_cast<uint8_t*>(aggregate_data[agg_idx]), nullptr);
		} else {
			aggregate_keys[agg_idx] = make_shared_ptr<GPUColumn>(count[0], aggregate_keys[agg_idx]->data_wrapper.type, reinterpret_cast<uint8_t*>(aggregate_data[agg_idx]), nullptr);
		}
	}
}
...
// 			group_by_keys[group] = make_shared_ptr<GPUColumn>(count[0], group_by_keys[group]->data_wrapper.type, reinterpret_cast<uint8_t*>(group_by_data[group]), reinterpret_cast<uint64_t*>(offset_data[group]), num_bytes[group], true, nullptr);
// 		} else {
// 			group_by_keys[group] = make_shared_ptr<GPUColumn>(count[0], group_by_keys[group]->data_wrapper.type, reinterpret_cast<uint8_t*>(group_by_data[group]), nullptr);
// 		}
// 		group_by_keys[group]->is_unique = old_unique;
...
HandleGroupByAggregateExpression(vector<shared_ptr<GPUColumn>> &group_by_keys, vector<shared_ptr<GPUColumn>> &aggregate_keys, GPUBufferManager* gpuBufferManager, const vector<unique_ptr<Expression>> &aggregates, int num_group_keys) {
	...
	if (string_groupby) {
    throw NotImplementedException("Group by on string columns disabled");
		...
		case GPUColumnTypeId::INT64:
			...
			} else if (aggregate_type.id() == GPUColumnTypeId::INT32) {
        ResolveTypeGroupByAggregateExpression<uint64_t, uint64_t>(group_by_keys, aggregate_keys, gpuBufferManager, aggregates, num_group_keys);
      } else {
        throw NotImplementedException("Unsupported sirius column type in `HandleGroupByAggregateExpression`: {}",
                                              static_cast<int>(aggregate_type.id()));
  ...
  if (column_size > 0) {
    ...
    if (aggregates.size() == 0) {
      ...
      if (group_by_column[0]->column_length > INT32_MAX ||
        ...
        //   "Group by column length or aggregate column length is too large for CuDF");
        if (can_use_sirius_impl) {
          HandleGroupByAggregateExpression(group_by_column,
                                aggregate_column,
                                gpuBufferManager,
                                aggregates,
                                num_group_keys);
          ...
          throw NotImplementedException(
            "Group by column length or aggregate column length is too large for CuDF and Sirius implementation cannot be used");
        }
      } else {
