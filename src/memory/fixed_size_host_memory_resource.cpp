...
fixed_size_host_memory_resource::allocate_multiple_blocks(std::size_t total_bytes) {
    ...
    if (total_bytes == 0) {
        // change nullptr to empty vector
        return sirius::make_unique<multiple_blocks_allocation>(sirius::vector<void*>(), this, block_size_);
    }
    ...
    return sirius::make_unique<multiple_blocks_allocation>(std::move(allocated_blocks), this, block_size_);
}
