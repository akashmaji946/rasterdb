  ...
  string function_name)
{
  ...
  shared_ptr<GPUColumn> result =
    make_shared_ptr<GPUColumn>(size, column->data_wrapper.type, reinterpret_cast<uint8_t*>(c), nullptr);
  return result;
}
  ...
  string function_name)
{
  ...
  shared_ptr<GPUColumn> result =
    make_shared_ptr<GPUColumn>(size, column1->data_wrapper.type, reinterpret_cast<uint8_t*>(c), nullptr);
  return result;
}
  ...
  switch (expr.expression_class) {
    case ExpressionClass::BOUND_BETWEEN: {
      ...
      shared_ptr<GPUColumn> materialized_column = HandleMaterializeExpression(
        input_relation.columns[bound_ref.index], gpuBufferManager);
    ...
    case ExpressionClass::BOUND_COMPARISON: {
      ...
      if (bound_comparison.right->expression_class == ExpressionClass::BOUND_CONSTANT) {
        ...
        if (input_relation.columns[bound_ref1.index]->data_wrapper.data == nullptr) {
          ...
          shared_ptr<GPUColumn> materialized_column = HandleMaterializeExpression(
            input_relation.columns[bound_ref1.index], gpuBufferManager);
          // count = gpuBufferManager->customCudaMalloc<uint64_t>(1, 0, 0);
        ...
        if (input_relation.columns[bound_ref1.index]->data_wrapper.data == nullptr ||
          ...
          shared_ptr<GPUColumn> materialized_column2 = HandleMaterializeExpression(
            input_relation.columns[bound_ref2.index], gpuBufferManager);
          // count = gpuBufferManager->customCudaMalloc<uint64_t>(1, 0, 0);
    ...
    case ExpressionClass::BOUND_FUNCTION: {
      ...
      if ((bound_function.ToString().find("substr") != std::string::npos) ||
        ...
        shared_ptr<GPUColumn> materialized_column =
          HandleMaterializeExpression(input_column, gpuBufferManager);
