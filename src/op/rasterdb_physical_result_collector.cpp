...
rasterdb_physical_result_collector::rasterdb_physical_result_collector(
  ::rasterdb::sirius_prepared_statement_data& data)
  : rasterdb_physical_operator(
      RasterDBPhysicalOperatorType::RESULT_COLLECTOR, {duckdb::LogicalType::BOOLEAN}, 0),
    statement_type(data.prepared->statement_type),
    properties(data.prepared->properties),
    plan(*data.rasterdb_physical_plan),
    names(data.prepared->names)
{
  this->types = data.prepared->types;
}

std::unique_ptr<operator_data> rasterdb_physical_result_collector::execute(
  const operator_data& input_data, rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_result_collector::execute"};
  return std::make_unique<operator_data>(input_data);
}

duckdb::vector<duckdb::const_reference<rasterdb_physical_operator>>
rasterdb_physical_result_collector::get_children() const
{
  return {plan};
}

void rasterdb_physical_result_collector::build_pipelines(
...
rasterdb_physical_materialized_collector::rasterdb_physical_materialized_collector(
  ::rasterdb::sirius_prepared_statement_data& data, duckdb::ClientContext& client_ctx)
  : rasterdb_physical_result_collector(data),
    _client_ctx(client_ctx),
    result_collection(duckdb::make_uniq<duckdb::ColumnDataCollection>(client_ctx, types))
{
}

duckdb::unique_ptr<duckdb::QueryResult> rasterdb_physical_materialized_collector::get_result(
...
void rasterdb_physical_materialized_collector::sink(const operator_data& input_data,
                                                  rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_materialized_collector::sink"};
  const auto& input_batches     = input_data.get_data_batches();
  using host_table_chunk_reader = ::rasterdb::op::result::host_table_chunk_reader;
  ...
  auto sink_single_batch = [this,
    ...
    if (data->get_current_tier() == cucascade::memory::Tier::GPU) {
      ...
      auto sirius_ctx  = _client_ctx.registered_state->Get<duckdb::RasterDBContext>("rasterdb_state");
