...
rasterdb_physical_filter::rasterdb_physical_filter(
  duckdb::vector<duckdb::LogicalType> types,
  duckdb::vector<duckdb::unique_ptr<duckdb::Expression>> select_list,
  duckdb::idx_t estimated_cardinality)
  : rasterdb_physical_operator(
      RasterDBPhysicalOperatorType::FILTER, std::move(types), estimated_cardinality)
{
...
std::unique_ptr<operator_data> rasterdb_physical_filter::execute(const operator_data& input_data,
                                                               rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_filter::execute"};
  const auto& input_batches = input_data.get_data_batches();

  // The executor uses the data_batch API to filter rows according to `expression`.
  duckdb::rasterdb::GpuExpressionExecutor gpu_expression_executor(*expression.get());
