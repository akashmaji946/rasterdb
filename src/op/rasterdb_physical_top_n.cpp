...
rasterdb_physical_top_n::rasterdb_physical_top_n(
  ...
  : rasterdb_physical_operator(
      RasterDBPhysicalOperatorType::TOP_N, std::move(types_p), estimated_cardinality),
...
std::unique_ptr<operator_data> rasterdb_physical_top_n::execute(const operator_data& input_data,
                                                              rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_top_n::execute"};
  ...
  outputs.push_back(
    std::make_shared<cucascade::data_batch>(::rasterdb::get_next_batch_id(), std::move(output_data)));
  return std::make_unique<operator_data>(outputs);
}

rasterdb_physical_top_n_merge::rasterdb_physical_top_n_merge(rasterdb_physical_top_n* top_n)
  : rasterdb_physical_top_n_merge(
...
rasterdb_physical_top_n_merge::rasterdb_physical_top_n_merge(
  duckdb::vector<duckdb::LogicalType> types_p,
  duckdb::vector<duckdb::BoundOrderByNode> orders,
  duckdb::idx_t limit,
  duckdb::idx_t offset,
  ...
  duckdb::idx_t estimated_cardinality)
  : rasterdb_physical_operator(
      RasterDBPhysicalOperatorType::MERGE_TOP_N, std::move(types_p), estimated_cardinality),
    orders(std::move(orders)),
    limit(limit),
    offset(offset),
    dynamic_filter(std::move(dynamic_filter_p))
...
std::unique_ptr<operator_data> rasterdb_physical_top_n_merge::execute(const operator_data& input_data,
                                                                    rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_top_n_merge::execute"};
  const auto& input_batches = input_data.get_data_batches();
  if (limit == 0) {
    return std::make_unique<operator_data>(std::vector<std::shared_ptr<cucascade::data_batch>>{});
  }
...
std::unique_ptr<operator_data> rasterdb_physical_top_n_merge::get_next_task_input_data()
{
