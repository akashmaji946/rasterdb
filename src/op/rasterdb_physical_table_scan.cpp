...
rasterdb_physical_table_scan::rasterdb_physical_table_scan(
  ...
  : rasterdb_physical_operator(
      RasterDBPhysicalOperatorType::TABLE_SCAN, std::move(types), estimated_cardinality),
...
std::unique_ptr<operator_data> rasterdb_physical_table_scan::execute(const operator_data& input_data,
                                                                   rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_table_scan::execute"};
  ...
  if (filter_expr != nullptr) {
    ...
    duckdb::rasterdb::GpuExpressionExecutor gpu_expression_executor(*filter_expr);
