...
rasterdb_physical_partition::rasterdb_physical_partition(duckdb::vector<duckdb::LogicalType> types,
                                                     duckdb::idx_t estimated_cardinality,
                                                     rasterdb_physical_operator* parent_op,
                                                     bool is_build,
                                                     uint64_t hash_partition_bytes)
  : rasterdb_physical_operator(
      RasterDBPhysicalOperatorType::PARTITION, std::move(types), estimated_cardinality)
{
...
std::string rasterdb_physical_partition::get_name() const { return "PARTITION"; }

bool rasterdb_physical_partition::is_source() const { return true; }

bool rasterdb_physical_partition::is_sink() const { return true; }

void rasterdb_physical_partition::get_partition_keys_and_type(rasterdb_physical_operator* op,
                                                            bool is_build)
{
  if (op->type == RasterDBPhysicalOperatorType::HASH_JOIN) {
    _partition_type    = PartitionType::HASH;
    auto& hash_join_op = op->Cast<rasterdb_physical_hash_join>();
  ...
  } else if (op->type == RasterDBPhysicalOperatorType::NESTED_LOOP_JOIN) {
    _partition_type = PartitionType::NONE;
    _num_partitions = 1;
  } else if (op->type == RasterDBPhysicalOperatorType::HASH_GROUP_BY) {
    _partition_type            = PartitionType::HASH;
    auto& grouped_aggregate_op = op->Cast<rasterdb_physical_grouped_aggregate>();
    ...
    //   }
    // }
  } else if (op->type == RasterDBPhysicalOperatorType::MERGE_GROUP_BY) {
    _partition_type                  = PartitionType::HASH;
    auto& grouped_aggregate_merge_op = op->Cast<rasterdb_physical_grouped_aggregate_merge>();
    _partition_keys                  = grouped_aggregate_merge_op.get_output_grouping_indices();

  } else if (op->type == RasterDBPhysicalOperatorType::CONCAT) {
    auto& parent_concat_op = op->Cast<rasterdb_physical_concat>();
    bool is_build          = parent_concat_op.is_build_concat();
    _is_build              = is_build;
    if (parent_concat_op.get_parent_op()->type == RasterDBPhysicalOperatorType::HASH_JOIN ||
        parent_concat_op.get_parent_op()->type == RasterDBPhysicalOperatorType::NESTED_LOOP_JOIN) {
...
bool rasterdb_physical_partition::is_build_partition() { return _is_build; }

std::unique_ptr<operator_data> rasterdb_physical_partition::execute(const operator_data& input_data,
                                                                  rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_partition::execute"};
  ...
  if (!_num_partitions.has_value()) {
    throw std::runtime_error("Num partitions was not set in rasterdb_physical_partition operator " +
                             std::to_string(this->get_operator_id()));
  }
...
void rasterdb_physical_partition::sink(const operator_data& input_data, rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_partition::sink"};
  const auto& input_batches = input_data.get_data_batches();
  ...
  for (auto& batch : input_batches) {
    for (auto& [next_op, port_id] : next_port_after_sink) {
      ...
      auto partition_consumer_op =
        dynamic_cast<rasterdb_physical_partition_consumer_operator*>(next_op);
...
int rasterdb_physical_partition::determine_num_partitions()
{
  if (ports.find("default") == ports.end()) {
    throw std::runtime_error(
      "rasterdb_physical_partition::determine_num_partitions() did not find default repo for id " +
      std::to_string(this->get_operator_id()));
  }
...
void rasterdb_physical_partition::set_num_partitions(int num_partitions)
{
  std::lock_guard<std::mutex> guard(lock);
  _num_partitions = num_partitions;
}

std::optional<task_creation_hint> rasterdb_physical_partition::get_next_task_hint()
{
  std::lock_guard<std::mutex> guard(lock);
  if (!_num_partitions.has_value() && !_is_build && _sibling_partition_op != nullptr) {
    ...
    if (ports.size() != 1) {  // Ensure that it only has one port
      throw std::runtime_error("rasterdb_physical_concat: there should be only one port");
    }
    ...
    return rasterdb_physical_operator::get_next_task_hint();
  }
}

std::unique_ptr<operator_data> rasterdb_physical_partition::get_next_task_input_data()
{
  ...
  if (_sibling_partition_op) {
    auto& sibling = _sibling_partition_op->Cast<rasterdb_physical_partition>();
    ...
    if (!_num_partitions.has_value()) {
      ...
      RASTERDB_LOG_DEBUG(
        "rasterdb_physical_partition id {} determined {} partitions on sibling id {} and {} build "
        "side",
    ...
    if (!_num_partitions.has_value()) {
      _num_partitions = determine_num_partitions();
      RASTERDB_LOG_DEBUG("rasterdb_physical_partition id {} determined {} partitions",
                       this->get_operator_id(),
                       _num_partitions.value());
    }
  }
  return rasterdb_physical_operator::get_next_task_input_data();
}
