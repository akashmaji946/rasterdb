...
rasterdb_physical_grouped_aggregate::rasterdb_physical_grouped_aggregate(
  duckdb::ClientContext& context,
  duckdb::vector<duckdb::LogicalType> types,
  duckdb::vector<duckdb::unique_ptr<duckdb::Expression>> expressions,
  duckdb::vector<duckdb::unique_ptr<duckdb::Expression>> groups_p,
  duckdb::idx_t estimated_cardinality)
  : rasterdb_physical_grouped_aggregate(context,
                                      std::move(types),
...
rasterdb_physical_grouped_aggregate::rasterdb_physical_grouped_aggregate(
  ...
  : rasterdb_physical_operator(
      RasterDBPhysicalOperatorType::HASH_GROUP_BY, std::move(types), estimated_cardinality),
...
std::unique_ptr<operator_data> rasterdb_physical_grouped_aggregate::execute(
  const operator_data& input_data, rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_grouped_aggregate::execute"};
