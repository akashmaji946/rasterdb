...
rasterdb_physical_merge_sort::rasterdb_physical_merge_sort(rasterdb_physical_order* order_by)
  : rasterdb_physical_merge_sort(
...
rasterdb_physical_merge_sort::rasterdb_physical_merge_sort(
  ...
  : rasterdb_physical_operator(
      RasterDBPhysicalOperatorType::MERGE_SORT, std::move(types), estimated_cardinality),
...
std::unique_ptr<operator_data> rasterdb_physical_merge_sort::get_next_task_input_data()
{
  // Drain all batches from one partition per call (one merge task per partition).
  // Follows the same pattern as rasterdb_physical_grouped_aggregate_merge.
  ...
  if (_current_partition_index < repo->num_partitions()) {
    ...
    RASTERDB_LOG_DEBUG("merge_sort: drained {} batches for partition {}",
                     all_batches.size(),
...
std::unique_ptr<operator_data> rasterdb_physical_merge_sort::execute(const operator_data& input_data,
                                                                   rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_merge_sort::execute"};
  ...
  auto apply_final_projection =
    ...
    return rasterdb::make_data_batch(std::move(projected_table), *space);
  };
