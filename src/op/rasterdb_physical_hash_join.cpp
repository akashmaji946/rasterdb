...
bool rasterdb_physical_hash_join::are_conditions_supported(
...
rasterdb_physical_hash_join::rasterdb_physical_hash_join(
  ...
  : rasterdb_physical_partition_consumer_operator(
      RasterDBPhysicalOperatorType::HASH_JOIN, op.types, estimated_cardinality),
...
//===--------------------------------------------------------------------===//
void rasterdb_physical_hash_join::build_join_pipelines(pipeline::rasterdb_pipeline& current,
                                                     ...
                                                     bool build_rhs)
{
  ...
  auto& join_op           = op.Cast<rasterdb_physical_hash_join>();
...
void rasterdb_physical_hash_join::build_pipelines(pipeline::rasterdb_pipeline& current,
                                                pipeline::rasterdb_meta_pipeline& meta_pipeline)
{
  rasterdb_physical_hash_join::build_join_pipelines(current, meta_pipeline, *this);
}

std::unique_ptr<operator_data> rasterdb_physical_hash_join::get_next_task_input_data()
{
  ...
  if (left_batch_ids.empty() && right_batch_ids.empty()) {
    if (ports["default"]->repo->num_partitions() != ports["build"]->repo->num_partitions()) {
      throw std::runtime_error(
        "In rasterdb_physical_hash_join:Number of partitions for left and right ports must be the "
        "same in operator " +
...
static join_keys_result prepare_join_keys(
  ...
  bool cast_necessary,
  const std::vector<rasterdb_physical_hash_join::key_cast_info>& key_casts,
...
std::unique_ptr<operator_data> rasterdb_physical_hash_join::execute(const operator_data& input_data,
                                                                  rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_hash_join::execute"};
  ...
  if (is_all_inequality_join) {
    throw std::runtime_error(
      "Error rasterdb_physical_hash_join being asked to do all inequality join of type: " +
  ...
  if (is_mixed_join) {
    ...
    rasterdb::gpu_expression_translator translator(stream, cudf::get_current_device_resource_ref());
    auto pred =
      translator.translate_join_conditions(conditions, num_equality_conditions, conditions.size());
    if (!pred) {
      throw std::runtime_error(
        "In rasterdb_physical_hash_join: failed to translate mixed join inequality conditions to "
        "cuDF AST predicate");
    }
    ...
    if (join_type == duckdb::JoinType::MARK) {
      ...
      if (!swapped_pred) {
        throw std::runtime_error(
          "In rasterdb_physical_hash_join: failed to translate swapped predicate for RIGHT mixed "
          "join");
      }
      ...
      if (!swapped_pred) {
        throw std::runtime_error(
          "In rasterdb_physical_hash_join: failed to translate swapped predicate for RIGHT_SEMI "
          "mixed join");
      }
      ...
      if (!swapped_pred) {
        throw std::runtime_error(
          "In rasterdb_physical_hash_join: failed to translate swapped predicate for RIGHT_ANTI "
          "mixed join");
      }
