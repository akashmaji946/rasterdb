...
rasterdb_physical_sort_sample::rasterdb_physical_sort_sample(rasterdb_physical_order* order_by)
  : rasterdb_physical_sort_sample(
      order_by->types, copy_orders(order_by->orders), order_by->estimated_cardinality)
{
}

rasterdb_physical_sort_sample::rasterdb_physical_sort_sample(
  duckdb::vector<duckdb::LogicalType> types,
  duckdb::vector<duckdb::BoundOrderByNode> orders,
  duckdb::idx_t estimated_cardinality,
  duckdb::idx_t num_sample_batches)
  : rasterdb_physical_operator(
      RasterDBPhysicalOperatorType::SORT_SAMPLE, std::move(types), estimated_cardinality),
    orders(std::move(orders)),
    num_sample_batches(num_sample_batches)
{
}

std::optional<task_creation_hint> rasterdb_physical_sort_sample::get_next_task_hint()
{
  // If boundaries already computed, use default behavior (process batches as they arrive)
  if (_boundaries_computed.load()) { return rasterdb_physical_operator::get_next_task_hint(); }
...
std::unique_ptr<operator_data> rasterdb_physical_sort_sample::execute(const operator_data& input_data,
                                                                    rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_sort_sample::execute"};
  const auto& input_batches = input_data.get_data_batches();

  // After boundaries are computed, just pass through
  if (_boundaries_computed.load()) {
    RASTERDB_LOG_DEBUG("Sort sample: passthrough ({} batches)", input_batches.size());
    return std::make_unique<operator_data>(input_data);
  }

  RASTERDB_LOG_DEBUG("Sort sample: computing partition boundaries from {} batches",
                   input_batches.size());
  ...
  RASTERDB_LOG_DEBUG("Sort sample: computed {} partitions with {} boundaries in {:.2f} ms",
                   _num_partitions,
