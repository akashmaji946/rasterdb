...
#include "op/rasterdb_physical_nested_loop_join.hpp"
...
rasterdb_physical_nested_loop_join::rasterdb_physical_nested_loop_join(
  ...
  : rasterdb_physical_partition_consumer_operator(
      RasterDBPhysicalOperatorType::NESTED_LOOP_JOIN, op.types, estimated_cardinality),
    join_type(join_type),
...
bool rasterdb_physical_nested_loop_join::is_supported(
...
duckdb::vector<duckdb::LogicalType> rasterdb_physical_nested_loop_join::get_join_types() const
{
...
//===--------------------------------------------------------------------===//
void rasterdb_physical_nested_loop_join::build_join_pipelines(
  pipeline::rasterdb_pipeline& current,
  ...
  bool build_rhs)
{
  ...
  duckdb::vector<duckdb::shared_ptr<pipeline::rasterdb_pipeline>> pipelines_so_far;
  meta_pipeline.get_pipelines(pipelines_so_far, false);
  auto& last_pipeline = *pipelines_so_far.back();

  duckdb::vector<duckdb::shared_ptr<pipeline::rasterdb_pipeline>> dependencies;
  duckdb::optional_ptr<pipeline::rasterdb_meta_pipeline> last_child_ptr;
  if (build_rhs) {
  ...
  switch (op.type) {
    case RasterDBPhysicalOperatorType::POSITIONAL_JOIN:
      ...
      return;
    case RasterDBPhysicalOperatorType::CROSS_PRODUCT:
  ...
  auto& join_op           = op.Cast<rasterdb_physical_nested_loop_join>();
...
void rasterdb_physical_nested_loop_join::build_pipelines(
  pipeline::rasterdb_pipeline& current, pipeline::rasterdb_meta_pipeline& meta_pipeline)
{
  rasterdb_physical_nested_loop_join::build_join_pipelines(current, meta_pipeline, *this);
}

std::unique_ptr<operator_data> rasterdb_physical_nested_loop_join::get_next_task_input_data()
{
  ...
  if (left_batch_ids.empty() && right_batch_ids.empty()) {
    ...
    if (default_port->repo->num_partitions() != build_port->repo->num_partitions()) {
      throw std::runtime_error(
        "rasterdb_physical_nested_loop_join: number of partitions for default and build ports must "
        "match");
    }
...
cudf::ast::ast_operator to_ast_operator(duckdb::ExpressionType comparison)
{
  switch (comparison) {
    ...
    default:
      throw std::runtime_error("rasterdb_physical_nested_loop_join: unsupported comparison type");
  }
}
...
std::unique_ptr<operator_data> rasterdb_physical_nested_loop_join::execute(
  const operator_data& input_data, rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_nested_loop_join::execute"};
  ...
  if (input_batches.size() != 2) {
    throw std::runtime_error(
      "rasterdb_physical_nested_loop_join expects 2 input batches (left, right), got " +
  ...
  if (!left_batch || !right_batch) {
    RASTERDB_LOG_DEBUG("Pipeline {}: nested loop join, 0 output batches", pipeline_id);
  ...
  if (left.num_rows() == 0 || right.num_rows() == 0) {
    ...
    RASTERDB_LOG_DEBUG("Pipeline {}: nested loop join, 1 empty output batches", pipeline_id);
    return std::make_unique<operator_data>(std::vector<std::shared_ptr<cucascade::data_batch>>{
  ...
  if (conditions.empty()) {
    ...
    auto resolve_join_col = [&](const duckdb::Expression& expr,
      ...
      if (!get_column_index(expr, source_idx)) {
        duckdb::rasterdb::GpuExpressionExecutor executor(expr, mr);
        ...
        if (expr_view.num_columns() != 1) {
          throw std::runtime_error(std::string("rasterdb_physical_nested_loop_join: expression on ") +
                                   side + " should produce one column");
        }
        if (expr_view.num_rows() != table.num_rows()) {
          throw std::runtime_error(
            std::string(
              "rasterdb_physical_nested_loop_join: expression result row count must match ") +
            side + " table");
        }
        ...
        if (table.column(source_idx).type() != target_type) {
          if (expr.expression_class != duckdb::ExpressionClass::BOUND_CAST) {
            ...
            throw std::runtime_error(
              "rasterdb_physical_nested_loop_join: unexpected, column type does not match, yet "
              "there "
              "is no BOUND_CAST");
          }
    ...
    switch (join_type) {
      ...
      case duckdb::JoinType::SEMI: {
        ...
        RASTERDB_LOG_DEBUG("Pipeline {}: nested loop join, 1 output batches", pipeline_id);
        return std::make_unique<operator_data>(std::vector<std::shared_ptr<cucascade::data_batch>>{
      ...
      case duckdb::JoinType::OUTER:
        ...
        break;
      default:
        throw std::runtime_error("rasterdb_physical_nested_loop_join: unsupported join type: " +
