    ...
    _selected_column_indices(detail::make_selected_column_indices(*scan_op))
{
  ...
  // Construct the file reader (doesn't yet need reader options) and read the metadata
...
void parquet_scan_task::execute()
{
  ...
  auto byte_ranges =
    reader->payload_column_chunks_byte_ranges(l_state.get_rg_span(), g_state.get_options());
