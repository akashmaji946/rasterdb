...
rasterdb_physical_order::rasterdb_physical_order(duckdb::vector<duckdb::LogicalType> types,
                                             duckdb::vector<duckdb::BoundOrderByNode> orders,
                                             duckdb::vector<duckdb::idx_t> projections_p,
                                             duckdb::idx_t estimated_cardinality,
                                             bool is_index_sort_p)
  : rasterdb_physical_operator(
      RasterDBPhysicalOperatorType::ORDER_BY, std::move(types), estimated_cardinality),
...
std::unique_ptr<operator_data> rasterdb_physical_order::execute(const operator_data& input_data,
                                                              rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_order::execute"};
