...
//===--------------------------------------------------------------------===//
// LCOV_EXCL_START
duckdb::unique_ptr<duckdb::OperatorState> rasterdb_physical_operator::get_operator_state(
  duckdb::ExecutionContext& context) const
{
  return duckdb::make_uniq<duckdb::OperatorState>();
}

duckdb::unique_ptr<duckdb::GlobalOperatorState> rasterdb_physical_operator::get_global_operator_state(
...
// Source
//===--------------------------------------------------------------------===//
duckdb::unique_ptr<duckdb::LocalSourceState> rasterdb_physical_operator::get_local_source_state(
  duckdb::ExecutionContext& context, duckdb::GlobalSourceState& gstate) const
{
  return duckdb::make_uniq<duckdb::LocalSourceState>();
}

duckdb::unique_ptr<duckdb::GlobalSourceState> rasterdb_physical_operator::get_global_source_state(
  duckdb::ClientContext& context) const
{
...
// Sink
//===--------------------------------------------------------------------===//
duckdb::unique_ptr<duckdb::LocalSinkState> rasterdb_physical_operator::get_local_sink_state(
  duckdb::ExecutionContext& context) const
{
  return duckdb::make_uniq<duckdb::LocalSinkState>();
}

duckdb::unique_ptr<duckdb::GlobalSinkState> rasterdb_physical_operator::get_global_sink_state(
  duckdb::ClientContext& context) const
{
  return duckdb::make_uniq<duckdb::GlobalSinkState>();
}

std::string rasterdb_physical_operator::get_name() const
{
  return RasterdbPhysicalOperatorToString(type);
}
...
{
  duckdb::vector<duckdb::const_reference<rasterdb_physical_operator>> result;
...
//===--------------------------------------------------------------------===//
void rasterdb_physical_operator::build_pipelines(pipeline::rasterdb_pipeline& current,
...
duckdb::vector<duckdb::const_reference<rasterdb_physical_operator>>
rasterdb_physical_operator::get_sources() const
{
  duckdb::vector<duckdb::const_reference<rasterdb_physical_operator>> result;
  if (is_sink()) {
...
void rasterdb_physical_operator::verify()
{
#ifdef DEBUG
...
void rasterdb_physical_operator::add_port(std::string_view port_id, std::unique_ptr<port> p)
{
...
rasterdb_physical_operator::port* rasterdb_physical_operator::get_port(std::string_view port_id)
{
...
void rasterdb_physical_operator::sink(const operator_data& output_data, rmm::cuda_stream_view stream)
{
...
std::unique_ptr<operator_data> rasterdb_physical_operator::execute(const operator_data& input_data,
...
void rasterdb_physical_operator::push_data_batch(std::string_view port_id,
...
void rasterdb_physical_operator::add_next_port_after_sink(
  std::pair<rasterdb_physical_operator*, std::string_view> port_locator)
{
  next_port_after_sink.push_back(port_locator);
}

std::vector<std::pair<rasterdb_physical_operator*, std::string_view>>&
rasterdb_physical_operator::get_next_port_after_sink()
{
  return next_port_after_sink;
}

std::optional<task_creation_hint> rasterdb_physical_operator::get_next_task_hint()
{
...
std::unique_ptr<operator_data> rasterdb_physical_operator::get_next_task_input_data()
{
...
bool rasterdb_physical_operator::all_ports_empty()
{
  for (auto& [port_name, port_ptr] : ports) {
...
bool rasterdb_physical_operator::is_source_pipeline_finished()
{
  for (auto& [port_name, port_ptr] : ports) {
...
bool rasterdb_physical_operator::has_full_barrier_from(const pipeline::rasterdb_pipeline* src) const
{
...
duckdb::shared_ptr<pipeline::rasterdb_pipeline> rasterdb_physical_operator::get_pipeline()
  const noexcept
{
  return _pipeline;
}

void rasterdb_physical_operator::set_pipeline(duckdb::shared_ptr<pipeline::rasterdb_pipeline> pipeline)
{
...
std::vector<std::string_view> rasterdb_physical_operator::get_port_ids()
{
