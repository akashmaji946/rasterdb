...
rasterdb_physical_sort_partition::rasterdb_physical_sort_partition(rasterdb_physical_order* order_by)
  : rasterdb_physical_sort_partition(order_by->types,
...
rasterdb_physical_sort_partition::rasterdb_physical_sort_partition(
  duckdb::vector<duckdb::LogicalType> types,
  duckdb::vector<duckdb::BoundOrderByNode> orders,
  duckdb::vector<duckdb::idx_t> projections_p,
  duckdb::idx_t estimated_cardinality)
  : rasterdb_physical_operator(
      RasterDBPhysicalOperatorType::SORT_PARTITION, std::move(types), estimated_cardinality),
    orders(std::move(orders)),
    projections(std::move(projections_p))
{
}

std::unique_ptr<operator_data> rasterdb_physical_sort_partition::execute(
  const operator_data& input_data, rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_sort_partition::execute"};
  ...
  if (!_sample_op || !_sample_op->boundaries_computed() || _sample_op->get_num_partitions() <= 1) {
    RASTERDB_LOG_DEBUG("Sort partition: passthrough ({} batches, {} partitions)",
                     input_batches.size(),
