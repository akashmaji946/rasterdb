...
rasterdb_physical_ungrouped_aggregate::rasterdb_physical_ungrouped_aggregate(
  duckdb::vector<duckdb::LogicalType> types,
  duckdb::vector<duckdb::unique_ptr<duckdb::Expression>> expressions,
  duckdb::idx_t estimated_cardinality,
  duckdb::TupleDataValidityType distinct_validity)
  : rasterdb_physical_operator(
      RasterDBPhysicalOperatorType::UNGROUPED_AGGREGATE, std::move(types), estimated_cardinality),
...
std::unique_ptr<operator_data> rasterdb_physical_ungrouped_aggregate::execute(
  const operator_data& input_data, rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_ungrouped_aggregate::execute"};
  ...
  for (auto const& batch : input_batches) {
    ...
    auto const batch_id = ::rasterdb::get_next_batch_id();
...
rasterdb_physical_ungrouped_aggregate_merge::rasterdb_physical_ungrouped_aggregate_merge(
  rasterdb_physical_ungrouped_aggregate* ungrouped_aggregate)
  : rasterdb_physical_ungrouped_aggregate_merge(
...
rasterdb_physical_ungrouped_aggregate_merge::rasterdb_physical_ungrouped_aggregate_merge(
  ...
  duckdb::TupleDataValidityType distinct_validity)
  : rasterdb_physical_operator(
      RasterDBPhysicalOperatorType::MERGE_AGGREGATE, std::move(types), estimated_cardinality),
    aggregates(std::move(expressions))
{
  distinct_collection_info = duckdb::DistinctAggregateCollectionInfo::Create(aggregates);
  // aggregation_result       = duckdb::make_shared_ptr<GPUIntermediateRelation>(aggregates.size());
...
std::unique_ptr<operator_data> rasterdb_physical_ungrouped_aggregate_merge::execute(
  const operator_data& input_data, rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_ungrouped_aggregate_merge::execute"};
  const auto& input_batches = input_data.get_data_batches();
  if (aggregates.empty()) {
    return std::make_unique<operator_data>(std::vector<std::shared_ptr<cucascade::data_batch>>{});
  }
...
std::unique_ptr<operator_data> rasterdb_physical_ungrouped_aggregate_merge::get_next_task_input_data()
{
