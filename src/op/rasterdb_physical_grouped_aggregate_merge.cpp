...
rasterdb_physical_grouped_aggregate_merge::rasterdb_physical_grouped_aggregate_merge(
  rasterdb_physical_grouped_aggregate* grouped_aggregate)
  : rasterdb_physical_grouped_aggregate_merge(grouped_aggregate->types,
...
rasterdb_physical_grouped_aggregate_merge::rasterdb_physical_grouped_aggregate_merge(
  ...
  : rasterdb_physical_partition_consumer_operator(
      RasterDBPhysicalOperatorType::MERGE_GROUP_BY, std::move(types), estimated_cardinality),
...
rasterdb_physical_grouped_aggregate_merge::rasterdb_physical_grouped_aggregate_merge(
  ...
  duckdb::idx_t estimated_cardinality)
  : rasterdb_physical_grouped_aggregate_merge(context,
                                            std::move(types),
...
std::unique_ptr<operator_data> rasterdb_physical_grouped_aggregate_merge::get_next_task_input_data()
{
...
std::unique_ptr<operator_data> rasterdb_physical_grouped_aggregate_merge::execute(
  const operator_data& input_data, rmm::cuda_stream_view stream)
{
  nvtx3::scoped_range nvtx_range{"rasterdb_physical_grouped_aggregate_merge::execute"};
  ...
  auto result       = rasterdb::make_data_batch(std::move(output_table), *space);
  return std::make_unique<operator_data>(
