...
void gpu_pipeline_task_queue::open() {
    sirius::lock_guard<sirius::mutex> lock(_mutex);
    _is_open = true;
}

void gpu_pipeline_task_queue::close() {
    // Wake up all waiting threads by releasing the semaphore enough times
    for (int i = 0; i < Config::NUM_GPU_PIPELINE_EXECUTOR_THREADS; ++i) {
        _sem.release();
    }
    sirius::lock_guard<sirius::mutex> lock(_mutex);
    _is_open = false;
...
void gpu_pipeline_task_queue::push(sirius::unique_ptr<itask> task) {
    ...
    enqueue_task(std::move(gpu_task));
}

sirius::unique_ptr<itask> gpu_pipeline_task_queue::pull() {
    // Delegate to GPU-specific version and return as base type
    auto gpu_task = dequeue_task();
    return std::move(gpu_task);
}

void gpu_pipeline_task_queue::enqueue_task(sirius::unique_ptr<gpu_pipeline_task> gpu_pipeline_task) {
    sirius::lock_guard<sirius::mutex> lock(_mutex);
    if (gpu_pipeline_task && _is_open) {
        _task_queue.push(std::move(gpu_pipeline_task));
    }
    _sem.release(); // signal that one item is available
}

sirius::unique_ptr<gpu_pipeline_task> gpu_pipeline_task_queue::dequeue_task() {
...
bool gpu_pipeline_task_queue::is_empty() const {
    sirius::lock_guard<sirius::mutex> lock(_mutex);
