...
rasterdb_meta_pipeline::rasterdb_meta_pipeline(
  rasterdb_engine& engine,
  rasterdb_pipeline_build_state& state_p,
  duckdb::optional_ptr<op::rasterdb_physical_operator> sink_p)
  : engine(engine), state(state_p), sink(sink_p), recursive_cte(false), next_batch_index(0)
{
  create_pipeline();
}

rasterdb_engine& rasterdb_meta_pipeline::get_engine() const { return engine; }

rasterdb_pipeline_build_state& rasterdb_meta_pipeline::get_state() const { return state; }

duckdb::optional_ptr<op::rasterdb_physical_operator> rasterdb_meta_pipeline::get_sink() const
{
  return sink;
}

duckdb::optional_ptr<rasterdb_pipeline> rasterdb_meta_pipeline::get_parent() const { return parent; }

duckdb::shared_ptr<rasterdb_pipeline>& rasterdb_meta_pipeline::get_base_pipeline()
{
  return pipelines[0];
}

void rasterdb_meta_pipeline::get_pipelines(
  duckdb::vector<duckdb::shared_ptr<rasterdb_pipeline>>& result, bool recursive)
{
...
void rasterdb_meta_pipeline::get_meta_pipelines(
  duckdb::vector<duckdb::shared_ptr<rasterdb_meta_pipeline>>& result, bool recursive, bool skip)
{
  if (!skip) {
    result.push_back(duckdb::enable_shared_from_this<rasterdb_meta_pipeline>::shared_from_this());
  }
  if (recursive) {
    for (auto& child : children) {
...
rasterdb_meta_pipeline& rasterdb_meta_pipeline::get_last_child()
{
  if (children.empty()) { return *this; }
  duckdb::reference<const duckdb::vector<duckdb::shared_ptr<rasterdb_meta_pipeline>>>
...
duckdb::optional_ptr<const duckdb::vector<duckdb::reference<rasterdb_pipeline>>>
rasterdb_meta_pipeline::get_dependencies(rasterdb_pipeline& dependent) const
{
...
bool rasterdb_meta_pipeline::has_recursive_cte() const { return recursive_cte; }

void rasterdb_meta_pipeline::set_recursive_cte() { recursive_cte = true; }

void rasterdb_meta_pipeline::assign_next_batch_index(rasterdb_pipeline& pipeline)
{
  pipeline.base_batch_index = next_batch_index++ * rasterdb_pipeline_build_state::BATCH_INCREMENT;
}

void rasterdb_meta_pipeline::build(op::rasterdb_physical_operator& op)
{
  D_ASSERT(pipelines.size() == 1);
  D_ASSERT(children.empty());
  // RASTERDB_LOG_DEBUG("op.type = {}", duckdb::PhysicalOperatorToString(op.type));
  op.build_pipelines(*pipelines.back(), *this);
}

void rasterdb_meta_pipeline::ready()
{
...
rasterdb_meta_pipeline& rasterdb_meta_pipeline::create_child_meta_pipeline(
  rasterdb_pipeline& current, op::rasterdb_physical_operator& op)
{
  children.push_back(duckdb::make_shared_ptr<rasterdb_meta_pipeline>(engine, state, &op));
  auto child_meta_pipeline = children.back().get();
  // store the parent
  child_meta_pipeline->parent = &current;
  // child rasterdb_meta_pipeline must finish completely before this rasterdb_meta_pipeline can start
...
rasterdb_pipeline& rasterdb_meta_pipeline::create_pipeline()
{
  pipelines.emplace_back(duckdb::make_shared_ptr<rasterdb_pipeline>(engine));
  state.set_pipeline_sink(*pipelines.back(), sink, next_batch_index++);
  return *pipelines.back();
}

void rasterdb_meta_pipeline::add_dependencies_from(rasterdb_pipeline& dependent,
                                                 rasterdb_pipeline& start,
                                                 bool including)
{
  // find 'start'
  ...
  duckdb::vector<duckdb::reference<pipeline::rasterdb_pipeline>> created_pipelines;
...
void rasterdb_meta_pipeline::add_recursive_dependencies(
  ...
  const rasterdb_meta_pipeline& last_child)
{
  ...
  duckdb::vector<duckdb::shared_ptr<rasterdb_meta_pipeline>> child_meta_pipelines;
...
void rasterdb_meta_pipeline::add_finish_event(rasterdb_pipeline& pipeline)
{
...
bool rasterdb_meta_pipeline::has_finish_event(rasterdb_pipeline& pipeline) const
{
  return finish_pipelines.find(pipeline) != finish_pipelines.end();
}

duckdb::optional_ptr<rasterdb_pipeline> rasterdb_meta_pipeline::get_finish_group(
  rasterdb_pipeline& pipeline) const
{
  auto it = finish_map.find(pipeline);
  return it == finish_map.end() ? nullptr : &it->second;
}

rasterdb_pipeline& rasterdb_meta_pipeline::create_union_pipeline(rasterdb_pipeline& current,
                                                             bool order_matters)
{
  ...
  // 'union_pipeline' inherits ALL dependencies of 'current' (within this rasterdb_meta_pipeline, and
...
void rasterdb_meta_pipeline::create_child_pipeline(rasterdb_pipeline& current,
                                                 ...
                                                 rasterdb_pipeline& last_pipeline)
{
  ...
  // child pipeline has a dependency (within this rasterdb_meta_pipeline on all pipelines that were
