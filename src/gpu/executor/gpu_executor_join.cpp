...
std::unique_ptr<gpu_table> gpu_executor::execute_join(duckdb::LogicalComparisonJoin& op)
{
  ...
  for (size_t ci = 0; ci < op.conditions.size(); ci++) {
    ...
    RASTERDB_LOG_DEBUG("[RDB_DEBUG]   cond[{}]: cmp={} left={} right={}",
                       ci, duckdb::ExpressionTypeToString(c.comparison),
                       c.left->ToString(), c.right->ToString());
  }
  ...
  RASTERDB_LOG_DEBUG("[RDB_DEBUG] JOIN keys: L col[{}] addr=0x{:x} size={}, R col[{}] addr=0x{:x} size={}",
                     static_cast<size_t>(left_key_idx), static_cast<uint64_t>(left_key_view.data()),
                     left_key_view.size(), static_cast<size_t>(right_key_idx),
                     static_cast<uint64_t>(right_key_view.data()), right_key_view.size());
