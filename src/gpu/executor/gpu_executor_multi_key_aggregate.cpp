...
{
  if (kind == TUPLE_GB_COUNT) { return rasterdf::type_id::INT32; }
...
{
  if ((kind == TUPLE_GB_SUM || kind == TUPLE_GB_MEAN) && value_type == rasterdf::type_id::FLOAT32) {
    return rasterdf::type_id::FLOAT64;
  }
  ...
  if (tuple_candidate && !aggregates.empty()) {
    ...
    if (tuple_candidate) {
      ...
      for (size_t a = 0; a < agg_kinds.size(); ++a) {
        dense_candidate = dense_candidate && dense_groupby_agg_supported(agg_kinds[a]);
      }
      ...
      if (dense_candidate) {
        ...
        for (size_t k = 0; k < num_group_cols; k++) {
          out_key_bufs.emplace_back(
            _ctx.workspace_mr(), static_cast<size_t>(dense_num_groups) * sizeof(int32_t), usage);
          key_descs[k] = {
        ...
        for (size_t a = 0; a < aggregates.size(); a++) {
          state_i32_bufs.emplace_back(
            _ctx.workspace_mr(), static_cast<size_t>(dense_state_entries) * sizeof(int32_t), usage);
          state_i64_bufs.emplace_back(
            _ctx.workspace_mr(), static_cast<size_t>(dense_state_entries) * sizeof(int64_t), usage);
          aux_i64_bufs.emplace_back(
            _ctx.workspace_mr(), static_cast<size_t>(dense_state_entries) * sizeof(int64_t), usage);
          out_i32_bufs.emplace_back(
            _ctx.workspace_mr(), static_cast<size_t>(dense_num_groups) * sizeof(int32_t), usage);
          out_i64_bufs.emplace_back(
            _ctx.workspace_mr(), static_cast<size_t>(dense_num_groups) * sizeof(int64_t), usage);
          out_f32_bufs.emplace_back(
            _ctx.workspace_mr(), static_cast<size_t>(dense_num_groups) * sizeof(float), usage);
        ...
        RASTERDB_LOG_INFO(
          "[Dense GB Profile] desc_upload_ms={:.2f} key_desc_bytes={} agg_desc_bytes={}",
      ...
      if (tuple_low_cardinality_contention && !force_tuple_key) {
        ...
        RASTERDB_LOG_DEBUG(
          "[Tuple GB] forced tuple-key path: dense-ineligible low-cardinality shape will use tuple "
          "hash");
      }
    }
    ...
    if (tuple_candidate) {
      ...
      if (can_range_estimate) {
        ...
        RASTERDB_LOG_DEBUG(
          "[Tuple GB] range_estimate={} target_slots={}", range_product, target_slots);
      }
      ...
      for (size_t k = 0; k < num_group_cols; k++) {
        out_key_bufs.emplace_back(
          _ctx.workspace_mr(),
          static_cast<size_t>(n) * rdf_type_size(input.col(group_col_indices[k]).type.id),
          usage);
        key_descs[k] = {
          input.col(group_col_indices[k]).address(),
          out_key_bufs.back().data(),
      ...
      for (size_t a = 0; a < aggregates.size(); a++) {
        state_i32_bufs.emplace_back(
          _ctx.workspace_mr(), static_cast<size_t>(table_size) * sizeof(int32_t), usage);
        state_i64_bufs.emplace_back(
          _ctx.workspace_mr(), static_cast<size_t>(table_size) * sizeof(int64_t), usage);
        aux_i64_bufs.emplace_back(
          _ctx.workspace_mr(), static_cast<size_t>(table_size) * sizeof(int64_t), usage);
        out_i32_bufs.emplace_back(
          _ctx.workspace_mr(), static_cast<size_t>(n) * sizeof(int32_t), usage);
        out_i64_bufs.emplace_back(
          _ctx.workspace_mr(), static_cast<size_t>(n) * sizeof(int64_t), usage);
        out_f32_bufs.emplace_back(
          _ctx.workspace_mr(), static_cast<size_t>(n) * sizeof(float), usage);
      ...
      rasterdf::device_buffer slot_hash(
        _ctx.workspace_mr(), static_cast<size_t>(table_size) * sizeof(uint64_t), usage);
      rasterdf::device_buffer slot_row(
        _ctx.workspace_mr(), static_cast<size_t>(table_size) * sizeof(uint32_t), usage);
      ...
      for (size_t a = 0; a < aggregates.size(); a++) {
        ...
        disp.fill_buffer(
          aux_i64_bufs[a].buffer(), 0, table_size * sizeof(int64_t), aux_i64_bufs[a].offset());
      }
      disp.batch_barrier_fill_to_compute();
      ...
      uint32_t num_unique_groups = 0;
      uint32_t overflow          = 0;
      unique_count.copy_to_host(
        &num_unique_groups, sizeof(uint32_t), _ctx.device(), _ctx.queue(), _ctx.command_pool());
      overflow_count.copy_to_host(
        &overflow, sizeof(uint32_t), _ctx.device(), _ctx.queue(), _ctx.command_pool());
      if (overflow != 0) {
        throw duckdb::NotImplementedException(
          "RasterDB GPU: GroupBy hash table overflow (%u rows, table=%u)", n, table_size);
      }
      ...
      for (size_t a = 0; a < aggregates.size(); a++) {
        ...
        output.columns[out_col_idx].type = {output_type};
        output.columns[out_col_idx].num_rows = ng;
        if (output_type == rasterdf::type_id::INT64 || output_type == rasterdf::type_id::FLOAT64) {
