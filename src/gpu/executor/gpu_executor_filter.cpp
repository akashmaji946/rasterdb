...
std::unique_ptr<gpu_table> gpu_executor::execute_filter(duckdb::LogicalFilter& op)
{
  ...
  for (size_t c = 0; c < input->num_columns(); c++) {
    RASTERDB_LOG_DEBUG("  input col {}: {} rows, type={}, addr={}",
                       c,
                       input->col(c).num_rows,
                       static_cast<int>(input->col(c).type.id),
                       input->col(c).address());
  }
  ...
  RASTERDB_LOG_DEBUG(
    "[RDB_DEBUG] filter: {} rows => {} rows", input->num_rows(), result->num_rows());
  return result;
}
...
std::unique_ptr<gpu_table> gpu_executor::apply_filter_mask(const gpu_table& input, gpu_column& mask)
{
  ...
  disp.fill_buffer(
    prefix_buf.buffer(), 0u, sizeof(uint32_t), prefix_buf.offset() + n * sizeof(uint32_t));
  ...
  total_sum_buf.copy_to_host(
    &output_count, sizeof(uint32_t), _ctx.device(), _ctx.queue(), _ctx.command_pool());
  ...
  if (output_count < 256) {
    ...
    for (size_t c = 0; c < input.num_columns(); c++) {
      ...
      result->columns[c] =
        allocate_column(_ctx, in_col.type, static_cast<rasterdf::size_type>(output_count));

      // Copy prefix to host to find which rows passed
      std::vector<uint32_t> host_prefix(prefix_count);
      prefix_buf.copy_to_host(host_prefix.data(),
                              prefix_count * sizeof(uint32_t),
                              _ctx.device(),
                              _ctx.queue(),
                              _ctx.command_pool());
      ...
      const_cast<rasterdf::device_buffer&>(in_col.data)
        .copy_to_host(
          host_input.data(), n * elem_size, _ctx.device(), _ctx.queue(), _ctx.command_pool());
      ...
      // Copy output to GPU
      result->columns[c].data.copy_from_host(host_output.data(),
                                             output_count * elem_size,
                                             _ctx.device(),
                                             _ctx.queue(),
                                             _ctx.command_pool());
    }
