...
RasterDBContext::RasterDBContext() = default;

RasterDBContext::~RasterDBContext() noexcept
{
  if (is_initialized_) { terminate(); }
}

void RasterDBContext::QueryBegin(ClientContext& context)
{
  // Reset operator ID counter so each query starts from 0
  rasterdb::op::rasterdb_physical_operator::next_operator_id.store(0);
...
void RasterDBContext::QueryEnd()
{
  ...
  if (data_repository_manager_) {
    ...
    for (auto const& info : leaked) {
      spdlog::warn(
        "RasterDBContext::QueryEnd: operator {} port '{}' still had {} un-consumed "
...
void RasterDBContext::QueryEnd(ClientContext& context) { QueryEnd(); }

void RasterDBContext::QueryEnd(ClientContext& context, optional_ptr<ErrorData> error)
{
  QueryEnd(context);
}

void RasterDBContext::initialize(const rasterdb::rasterdb_config& config)
{
  if (is_initialized_) { throw std::runtime_error("RasterDB context is already initialized."); }

  config_ = config;

  memory_manager_ = std::make_unique<rasterdb::memory::rasterdb_memory_reservation_manager>(
  ...
  // the pageable H2D transfers that cuDF issues by default.
  {
    ...
    if (!host_spaces.empty()) {
      ...
      if (fsmr != nullptr) {
        ...
        spdlog::info("RasterDBContext: cuDF pinned memory resource configured (max slab {} B)",
                     cucascade::memory::small_pinned_host_memory_resource::MAX_SLAB_SIZE);
      }
    }
  }
  ...
  pipeline_executor_ = std::make_unique<rasterdb::pipeline::pipeline_executor>(
  ...
  auto create_executors_for_tier = [&](cucascade::memory::Tier tier) {
    ...
    for (auto* space : spaces) {
      rasterdb::parallel::task_executor_config executor_config{
        dg_cfg.num_threads, false, dg_cfg.cpu_affinity_list};
      auto executor = std::make_unique<rasterdb::parallel::downgrade_executor>(
  ...
  task_creator_ = std::make_unique<rasterdb::creator::task_creator>(config_.get_task_creator_config(),
                                                                  *memory_manager_);
...
void RasterDBContext::terminate()
{
  ...
  // can deadlock against a new cudaHostAlloc from the next RasterDBContext.
...
rasterdb::memory::rasterdb_memory_reservation_manager& RasterDBContext::get_memory_manager()
{
  throw_if_not_initialized();
  return *memory_manager_;
}

const rasterdb::memory::rasterdb_memory_reservation_manager& RasterDBContext::get_memory_manager() const
{
  throw_if_not_initialized();
  return *memory_manager_;
}

cucascade::shared_data_repository_manager& RasterDBContext::get_data_repository_manager()
{
  throw_if_not_initialized();
  return *data_repository_manager_;
}

const cucascade::shared_data_repository_manager& RasterDBContext::get_data_repository_manager() const
{
  throw_if_not_initialized();
  return *data_repository_manager_;
}

rasterdb::pipeline::pipeline_executor& RasterDBContext::get_pipeline_executor()
{
  throw_if_not_initialized();
  return *pipeline_executor_;
}

const rasterdb::pipeline::pipeline_executor& RasterDBContext::get_pipeline_executor() const
{
  throw_if_not_initialized();
  return *pipeline_executor_;
}

rasterdb::parallel::downgrade_executor& RasterDBContext::get_downgrade_executor(
...
const rasterdb::parallel::downgrade_executor& RasterDBContext::get_downgrade_executor(
...
const std::vector<std::unique_ptr<rasterdb::parallel::downgrade_executor>>&
RasterDBContext::get_downgrade_executors() const
{
  throw_if_not_initialized();
  return downgrade_executors_;
}

rasterdb::creator::task_creator& RasterDBContext::get_task_creator()
{
  throw_if_not_initialized();
  return *task_creator_;
}

const rasterdb::creator::task_creator& RasterDBContext::get_task_creator() const
{
  throw_if_not_initialized();
  return *task_creator_;
}

void RasterDBContext::create_query(rasterdb::rasterdb_pipeline_hashmap pipeline_hashmap)
{
  throw_if_not_initialized();
  query_ = duckdb::make_shared_ptr<rasterdb::planner::query>(std::move(pipeline_hashmap));
  pipeline_executor_->prepare_for_query(query_);
  task_creator_->prepare_for_query(*query_);
}

duckdb::shared_ptr<rasterdb::planner::query> RasterDBContext::get_query()
{
  throw_if_not_initialized();
  return query_;
}

duckdb::shared_ptr<const rasterdb::planner::query> RasterDBContext::get_query() const
{
  throw_if_not_initialized();
  return query_;
}

void RasterDBContext::throw_if_not_initialized() const
{
  if (!is_initialized_) { throw std::runtime_error("RasterDB context is not initialized."); }
}

// ================= Free Functions ================= //

RasterDBContextExtensionCallback::RasterDBContextExtensionCallback()
{
  InitGlobalLogger();
  read_config_file_if_exists();
}

void RasterDBContextExtensionCallback::OnConnectionOpened(ClientContext& context)
{
  spdlog::info("Connection opened.");
  if (context_) { context.registered_state->Insert("rasterdb_state", context_); }
}

void RasterDBContextExtensionCallback::OnConnectionClosed(ClientContext& context)
{
  spdlog::info("Connection closed.");
  // remove the context from the registered state
  context.registered_state->Remove("rasterdb_state");
}

void RasterDBContextExtensionCallback::OnExtensionLoaded(DatabaseInstance& db, const string& name)
{
  spdlog::info("Extension loaded: {}", name);
}

void RasterDBContextExtensionCallback::OnBeginExtensionLoad(DatabaseInstance& db, const string& name)
{
  spdlog::info("Beginning to load extension: {}", name);
}

void RasterDBContextExtensionCallback::OnExtensionLoadFail(DatabaseInstance& db,
                                                         const string& name,
...
void RasterDBContextExtensionCallback::read_config_file_if_exists()
{
  auto config_path = get_config_file_path();
  if (!std::filesystem::exists(config_path)) {
    spdlog::info("RasterDB configuration file does not exist at path: {}. Skipping loading.",
                 config_path);
    return;
  }
  ...
  extension_lock_ = std::make_unique<rasterdb::extension_lock>("sirius", lock_prefix);
  context_        = duckdb::make_shared_ptr<RasterDBContext>();
