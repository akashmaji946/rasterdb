    ...
    if (expr.children.size() == 3 && expr.children[2]->type == ExpressionType::CONJUNCTION_AND) {
      ...
      if (expr2.children.size() == 5 && expr2.children[0]->type == ExpressionType::COMPARE_EQUAL &&
        ...
        if (expr3.children.size() == 5) {
          ...
          // 12, 13))))";
          string t =
            "(((p_brand = 12) AND (p_size <= 5) AND (l_quantity <= 11) AND (p_container IN (0, "
            "1, 4, 5))) OR ((p_brand = 23) AND (p_size <= 10) AND (l_quantity >= 10) AND "
            "(l_quantity <= 20) AND (p_container IN (17, 18, 20, 21))) OR ((p_brand = 34) AND "
            "(p_size <= 15) AND (l_quantity >= 20) AND (l_quantity <= 30) AND (p_container IN "
            "(8, 9, 12, 13))))";
          if (!expression.ToString().compare(t)) {
            ...
            uint64_t* b = reinterpret_cast<uint64_t*>(materialized_quantity->data_wrapper.data);
