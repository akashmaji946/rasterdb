...
static bool can_use_partitioned_aggregate(duckdb::ClientContext& context,
                                          ...
                                          rasterdb::op::rasterdb_physical_operator& child,
                                          duckdb::vector<duckdb::column_t>& partition_columns)
{
  ...
  duckdb::reference<rasterdb::op::rasterdb_physical_operator> child_ref(child);
  while (child_ref.get().type != rasterdb::op::RasterDBPhysicalOperatorType::TABLE_SCAN) {
    auto& child_op = child_ref.get();
    switch (child_op.type) {
      case rasterdb::op::RasterDBPhysicalOperatorType::PROJECTION: {
        // recompute partition columns
        auto& projection = child_op.Cast<rasterdb::op::rasterdb_physical_projection>();
      ...
      case rasterdb::op::RasterDBPhysicalOperatorType::FILTER:
  ...
  auto& table_scan = child_ref.get().Cast<rasterdb::op::rasterdb_physical_table_scan>();
...
rasterdb_physical_plan_generator::create_plan(duckdb::LogicalAggregate& op)
{
  ...
  if (op.groups.empty() && op.grouping_sets.size() <= 1) {
    ...
    if (can_use_simple_aggregation) {
      auto group_by = duckdb::make_uniq_base<rasterdb::op::rasterdb_physical_operator,
                                             rasterdb::op::rasterdb_physical_ungrouped_aggregate>(
    ...
    // auto &group_by =
    //     Make<rasterdb::op::rasterdb_physical_grouped_aggregate>(context, op.types,
  ...
  if (can_use_simple_aggregation &&
    ...
    auto group_by = duckdb::make_uniq_base<rasterdb::op::rasterdb_physical_operator,
                                           rasterdb::op::rasterdb_physical_grouped_aggregate>(
      context,
      op.types,
...
duckdb::unique_ptr<rasterdb::op::rasterdb_physical_operator>
rasterdb_physical_plan_generator::extract_aggregate_expressions(
  ...
  duckdb::optional_ptr<duckdb::vector<duckdb::GroupingSet>> grouping_sets)
{
  ...
  auto projection = duckdb::make_uniq<rasterdb::op::rasterdb_physical_projection>(
