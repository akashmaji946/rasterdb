...
rasterdb_physical_plan_generator::plan_comparison_join(duckdb::LogicalComparisonJoin& op)
{
  ...
  bool is_supported_by_hash_join =
    rasterdb::op::rasterdb_physical_hash_join::are_conditions_supported(op.conditions);
  if (is_supported_by_hash_join && !prefer_range_joins) {
    ...
    // return join;
    auto join =
      duckdb::make_uniq<rasterdb::op::rasterdb_physical_hash_join>(op,
                                                               std::move(left),
                                                               std::move(right),
                                                               ...
                                                               std::move(op.filter_pushdown));
    join->Cast<rasterdb::op::rasterdb_physical_hash_join>().join_stats = std::move(op.join_stats);
    return join;
  }
  ...
  if (duckdb::PhysicalNestedLoopJoin::IsSupported(op.conditions, op.join_type)) {
    ...
    auto join =
      duckdb::make_uniq<rasterdb::op::rasterdb_physical_nested_loop_join>(op,
                                                                      std::move(left),
                                                                      std::move(right),
                                                                      std::move(op.conditions),
                                                                      op.join_type,
...
duckdb::unique_ptr<rasterdb::op::rasterdb_physical_operator>
rasterdb_physical_plan_generator::create_plan(duckdb::LogicalComparisonJoin& op)
{
  switch (op.type) {
