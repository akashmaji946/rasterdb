...
rasterdb_physical_plan_generator::create_plan(duckdb::LogicalColumnDataGet& op)
{
  D_ASSERT(op.children.size() == 0);
  D_ASSERT(op.collection);

  return duckdb::make_uniq<rasterdb::op::rasterdb_physical_column_data_scan>(
    op.types,
    rasterdb::op::RasterDBPhysicalOperatorType::COLUMN_DATA_SCAN,
