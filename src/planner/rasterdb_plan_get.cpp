...
rasterdb_physical_plan_generator::create_plan(duckdb::LogicalGet& op)
{
  ...
  duckdb::unique_ptr<rasterdb::op::rasterdb_physical_operator> filter;
  ...
  if (table_filters && op.function.supports_pushdown_type) {
    ...
    if (!select_list.empty()) {
      ...
      filter = duckdb::make_uniq<rasterdb::op::rasterdb_physical_filter>(
  ...
  if (!op.function.projection_pushdown) {
    ...
    auto node =
      duckdb::make_uniq<rasterdb::op::rasterdb_physical_table_scan>(op.returned_types,
                                                                op.function,
    ...
    duckdb::unique_ptr<rasterdb::op::rasterdb_physical_projection> projection =
      duckdb::make_uniq<rasterdb::op::rasterdb_physical_projection>(
  ...
  auto node = duckdb::make_uniq<rasterdb::op::rasterdb_physical_table_scan>(
