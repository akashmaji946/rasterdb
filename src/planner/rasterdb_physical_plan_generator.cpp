...
rasterdb_physical_plan_generator::rasterdb_physical_plan_generator(duckdb::ClientContext& context)
  : context(context)
{
}

rasterdb_physical_plan_generator::~rasterdb_physical_plan_generator() {}

duckdb::OrderPreservationType rasterdb_physical_plan_generator::order_preservation_recursive(
  rasterdb::op::rasterdb_physical_operator& op)
{
  ...
  for (auto& child : op.children) {
    ...
    if (op.type == rasterdb::op::RasterDBPhysicalOperatorType::CTE && child_idx == 0) {
      child_idx++;
      continue;
    }
...
bool rasterdb_physical_plan_generator::preserve_insertion_order(
  duckdb::ClientContext& context, rasterdb::op::rasterdb_physical_operator& plan)
{
...
rasterdb_physical_plan_generator::create_plan(duckdb::unique_ptr<duckdb::LogicalOperator> op)
{
...
duckdb::unique_ptr<rasterdb::op::rasterdb_physical_operator>
rasterdb_physical_plan_generator::create_plan(duckdb::LogicalOperator& op)
{
  RASTERDB_LOG_DEBUG("Creating sirius physical plan for logical operator type: {}",
                   duckdb::LogicalOperatorToString(op.type));
  op.estimated_cardinality                                      = op.EstimateCardinality(context);
  duckdb::unique_ptr<rasterdb::op::rasterdb_physical_operator> plan = nullptr;
  ...
  switch (op.type) {
    ...
    case duckdb::LogicalOperatorType::LOGICAL_EXTENSION_OPERATOR:
      ...
      // if (!plan) {
      // 	throw duckdb::InternalException("Missing rasterdb_physical_operator for Extension
      // Operator");
      // }
      break;
