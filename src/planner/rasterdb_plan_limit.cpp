...
rasterdb_physical_plan_generator::create_plan(duckdb::LogicalLimit& op)
{
  D_ASSERT(op.children.size() == 1);

  auto plan = create_plan(*op.children[0]);

  duckdb::unique_ptr<rasterdb::op::rasterdb_physical_operator> limit;
  switch (op.limit_val.Type()) {
    ...
    default:
      ...
      limit =
        duckdb::make_uniq<rasterdb::op::rasterdb_physical_streaming_limit>(op.types,
