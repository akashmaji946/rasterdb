...
**Coordinator Thread** - The main thread running Sirius extension on DuckDB. The thread coordinator will pass the logical plan information from DuckDB to the Pipeline Metadata Hash Map and signal Task Creator to start creating tasks.
...
**Downgrade Executor Thread** - A thread which has a stream associated with it and can be used to Downgrade Data Batches. It pulls from the Downgrade Task queue in order to get tasks to process.

**Downgrade Executor Thread Pool** - A pool of Downgrade Executor threads that exist in the Downgrade executor and are used to process Downgrade tasks as they are added to the queue. The number of threads here define the number of concurrent downgrades
...
**GPU Allocator** - A wrapper around RMM to allocate device memory.
...
**Memory Resource** - A wrapper around RMM memory resource which holds and manage host/device memory allocation. The memory resource can also be used to allocate new host/device memory.
...
**Pipeline Executor Thread** - A thread which has a stream associated with it and can be used to execute tasks on a GPU. It pulls from the pipeline queue in order to get tasks to process.

**Pipeline Executor Thread Pool** - A pool of Pipeline Executor threads that exist in the pipeline executor and are used to process tasks as they are added to the queue. The number of threads here define the number of concurrent running Pipeline Task.
...
**Scan Executor Thread** - A thread which can be used to scan tasks by calling DuckDB scan functions. It pulls from the Scan Task queue in order to get tasks to process.

**Scan Executor Thread Pool** - A pool of Scan Executor threads that exist in the Scan executor and are used to process scan tasks as they are added to the queue. The number of threads here define the parallelism of the scan.
