 ...
 * tests of the same type share a single DuckDB/RasterDBContext instance without
...
struct shared_env_listener : Catch::TestEventListenerBase {
  ...
  void testCaseStarting(Catch::TestCaseInfo const& info) override
  {
    ...
    if (needs != env_need::SHARED && rasterdb::test::g_shared_env &&
        rasterdb::test::g_shared_env->is_active()) {
      rasterdb::test::g_shared_env->pause();
    }
    if (needs != env_need::INTEGRATION && rasterdb::test::g_integration_env &&
        rasterdb::test::g_integration_env->is_active()) {
      rasterdb::test::g_integration_env->pause();
    }

    // Resume the environment this test needs
    if (needs == env_need::SHARED && rasterdb::test::g_shared_env &&
        !rasterdb::test::g_shared_env->is_active()) {
      rasterdb::test::g_shared_env->resume();
    }
    if (needs == env_need::INTEGRATION && rasterdb::test::g_integration_env &&
        !rasterdb::test::g_integration_env->is_active()) {
      rasterdb::test::g_integration_env->resume();
    }
  }
};
...
int main(int argc, char* argv[])
{
  ...
  std::string log_dir = RASTERDB_UNITTEST_LOG_DIR;
  ...
  auto scan_config_path =
    std::filesystem::path(RASTERDB_PROJECT_ROOT) / "test" / "cpp" / "scan" / "memory.cfg";
  rasterdb::test::shared_test_env scan_env(scan_config_path);
  scan_env.pause();
  rasterdb::test::g_shared_env = &scan_env;

  auto integration_config_path =
    std::filesystem::path(RASTERDB_PROJECT_ROOT) / "test" / "cpp" / "integration" / "integration.cfg";
  rasterdb::test::shared_test_env integration_env(integration_config_path);
  integration_env.pause();
  rasterdb::test::g_integration_env = &integration_env;

  Catch::Session session;
  session.applyCommandLine(argc, argv);
  int result = session.run();

  rasterdb::test::g_integration_env = nullptr;
  rasterdb::test::g_shared_env      = nullptr;
